...
function guiCreateWindow(x,y,g,u,yazi,relative,parent,renk1,renk2,renk3,renk4,kapatkutu)
	wsayi = #wtablo +1
	
	if not renk1 or string.len(renk1) > 6 then
		renk1 =  "222222" -- window renk üst taraf //  window top color
	end
	if not renk2 or string.len(renk2) > 6 then
		renk2 = "222222" -- window renk alt taraf // window bottom color
	end
	if not renk3 or string.len(renk3) > 6 then
		renk3 = "111111" -- baslik renk // window header back color
	end
	if not renk4 or string.len(renk4) > 6 then
		renk4 = "ffffff"  -- window kenar çizgi // window outline color
	end
	...
	for i,v in pairs(wtablo[wsayi].kenarlar) do
		renkVer(v,renk4)
		guiSetProperty(v, "DevOnTop", "True")
		guiSetAlpha(v, 0.4)
	end	
	--baslıkLabel
	...
	guiLabelSetColor(wtablo[wsayi].label, 255, 170, 0)
...
function guiCreateButton(x,y,g,u,yazi,relative,parent,renk1,renk2)
	...
	if not renk1 or string.len(renk1) > 6 then
		...
		renk1 = "111111" -- genel buton rengi // button back color
	end
	if not renk2 or string.len(renk2) > 6 then
		renk2 = "333333" -- buton kenar rengi // button outline color
	end
	--arkaResim
	if relative  then
		px,pu = guiGetSize(parent,false)
...
function guiCreateGridList(x,y,g,u,relative,parent,kenarrenk)
	...
	if not kenarrenk or string.len(kenarrenk) > 6 then
		kenarrenk =  "0055FF" -- gridlist kenar renk // gridlist outline color
	end
...
function guiCreateEdit(x,y,g,u,yazi,relative,parent,kenarrenk)
	...
	if not kenarrenk or string.len(kenarrenk) > 6 then
		kenarrenk =  "0055FF" -- edit kenar renk // edit outline color
	end
...
function guiCreateMemo(x,y,g,u,yazi,relative,parent,kenarrenk)
	msayi = #mtablo +1
	if not kenarrenk or string.len(kenarrenk) > 6 then
		kenarrenk =  "0055FF" -- memo kenar renk // memo outline color
	end
...
--basinca olan ufalma // anim when click
basili = {}
