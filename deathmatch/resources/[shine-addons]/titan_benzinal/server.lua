...
--[[
function benzinyardim(thePlayer)
  if benzincol then
   if isPedInVehicle then
   outputChatBox("#FF6600[?]#6699CC Benzin almak için /benzinal [Miktar(L)]", thePlayer, 255, 255, 255, true )
end
end
end
addEventHandler("onColShapeHit",benzincol ,benzinyardim)
]]

function benzinkomut(thePlayer)
  if benzincol then
    if isPedInVehicle then
  addCommandHandler("benzinal", benzinfunct)
end
end
end
...
function benzinkomut2(thePlayer)
  if benzincol then
    if isPedInVehicle then
  removeCommandHandler("benzinal", benzinfunct)
end
end
end
...
if not (id) then
			outputChatBox("#FF0000[!]#f6f6f6 Benzin Komut Kullanımı: /" .. commandName .. " [Miktar(L)]", thePlayer, 255, 194, 14, true)
		   else
		   local benzinmiktar = tonumber(id)
		   local veh = getPedOccupiedVehicle(thePlayer)
		  if benzinmiktar > 100 then
		   outputChatBox("#FF0000[!]##f6f6f6 Miktar alanına 100(yüz) den büyük bir sayı gelemez.",thePlayer, 255, 255, 255, true)
		   else
		   local veh = getPedOccupiedVehicle(thePlayer)
		   local cbenzin = getElementData(veh, "fuel")
		   olacak = cbenzin + id
		   if olacak > 100 then
		   outputChatBox("#FF0000[!]#f6f6f6 Benzin deposu bu kadar benzin alamaz.",thePlayer, 255, 255, 255, true)
		   else
		if isPedInVehicle(thePlayer) then
			local veh = getPedOccupiedVehicle(thePlayer)
			if (veh) then
...
local para = exports.global:getMoney(thePlayer)
 ...
 exports.global:takeMoney(thePlayer, id * 63)
...
setTimer ( function()
        odepara = id * 63
        outputChatBox("#66FF00[!]#f6f6f6 ".. odepara .."Tl'ye #FFFFFF".. benzinmiktar .." #f6f6f6Litre benzin dolduruldu.", thePlayer, 255, 255, 255, true)
		toggleControl(thePlayer, 'brake_reverse', true)
        setVehicleEngineState(veh, true)
    end, 5000, 1 )
	else
	outputChatBox("#FF0000[!]#f6f6f6 Malesef, Bu miktarda benzin için #FF0000".. gerekpara .."$ #6699CCgerekmekte.",thePlayer, 255, 255, 255, true)
end
end
end
end
