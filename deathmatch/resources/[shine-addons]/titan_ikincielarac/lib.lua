...
font3 = guiCreateFont("dosyalar/Font1.ttf",10)
...
function guiCreateWindow(x,y,g,u,yazi,relative,parent,renk1,renk2,renk3,renk4)
	...
	--kapatArka
	wtablo[wsayi].kapatArka = guiCreateStaticImage(g-25,0,25,20,resimOlustur("test"),false,wtablo[wsayi].basarka)
	...
	guiSetFont(wtablo[wsayi].kapat, font1)
...
function guiCreateButton(x,y,g,u,yazi,relative,parent,renk1,renk2)
	...
	guiSetFont(btablo[bsayi].label, font3)
...
function guiCreateGridList(x,y,g,u,relative,parent,kenarrenk)
	...
	Ltablo[Ssayi].liste = _guiCreateGridList(-8,-8,g+20, u+15,false, Ltablo[Ssayi].resim)
...
function guiEditSetColor(edit,kenarrenk)
	if editmi(edit) then
		local sira = editmi(edit)
		for i,v in pairs(etablo[sira].kenarlar) do
			renkVer(v,kenarrenk)
			guiSetAlpha(v, 0.5)
		end	
	end
