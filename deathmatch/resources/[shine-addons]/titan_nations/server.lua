...
function setUlke(thePlayer, commandName, targetPlayer, ulke)
	...
	if (not tonumber(ulke)) then
		...
		if targetPlayer then
			...
			mysql:query_fetch_assoc( "UPDATE `characters` SET `ulke`="..mysql:escape_string(ulke).." WHERE `id`=" .. mysql:escape_string(dbid) )
		end
			else
...
function turkiye(thePlayer)
	local dbid = getElementData(thePlayer, "dbid")
	ulke = 47
	setElementData(thePlayer, "ulke", tonumber(ulke))
	mysql:query_fetch_assoc( "UPDATE `characters` SET `ulke`="..mysql:escape_string(ulke).." WHERE `id`=" .. mysql:escape_string(dbid) )
end
addEvent("turkiye",true)
addEventHandler("turkiye",root,turkiye)
