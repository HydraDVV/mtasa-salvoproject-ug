...
function characterList( theClient )
	...
	local result = mysql:query("SELECT id,karakter_tip, charactername, cked, lastarea, age, weight, height, `description`, gender, faction_id, faction_rank, skin, DATEDIFF(NOW(), lastlogin) as llastlogin, ifnull(sehir, 'Secilmedi') as sehir_furkan FROM characters WHERE account='" .. mysql:escape_string(clientAccountID) .. "' AND `active` = 1 ORDER BY cked ASC, lastlogin DESC")
	...
	if (mysql:num_rows(result) > 0) then
		...
		while true do
			...
			characters[i][13] = tostring(row["sehir_furkan"])
			characters[i][14] = tostring(row["karakter_tip"])
			i = i + 1
		end
	end
...
function spawnCharacter(characterID)
	...
	local mailadresi = getElementDataEx(client, "mail")
	local characterData = mysql:query_fetch_assoc("SELECT *, ifnull(sehir, 'Secilmedi') as sehir_furkan FROM `characters` WHERE `id`='" .. tostring(characterID) .. "' AND `account`='" .. tostring(accountID) .. "' AND `cked`=0")
	if characterData then
		...
		setElementDataEx(client, "karakter_tip", tonumber(characterData["karakter_tip"]), true)
		setElementDataEx(client, "karakter:materyal", tonumber(characterData["materyal"]), true)
		setElementDataEx(client, "exp", tonumber(characterData["exp"]), true)
		setElementDataEx(client, "etiket", tonumber(characterData["etiket"]), true)
		setElementDataEx(client, "exprange", tonumber(characterData["exprange"]))
		--setElementDataEx(client, "vipver", tonumber(characterData["vipver"]), true)
		--setElementDataEx(client, "wearableitems", fromJSON(characterData["witems"]), true)
		setElementDataEx(client, "saglik:uyusturucu_bagimlilik", tonumber(characterData["bagimlilik"]), true)
		if tonumber(characterData["bagimlilik"]) > 0 then
			setTimer(setElementData, 30*30*1000, 1, client, "uyusturucu_kriz", true)
		end
		setElementDataEx(client, "ulke", tonumber(characterData["ulke"]), true)
		setElementDataEx(client, "dead", tonumber(characterData["dead"]))
		setElementDataEx(client, "samp", 0)
		setElementDataEx(client, "vipver", tonumber(characterData["vip"]), true)
		setElementDataEx(client, "suan", characterData["suan"], true)
		setElementDataEx(client, "bitis", characterData["bitis"], true)
		setElementDataEx(client, "silahhak", tonumber(characterData["silahhak"]), true)
		if tonumber(characterData["dead"]) == 1 then
			triggerEvent("bayilma:Bayilt", client, client)
		end
		...
		if not tonumber(characterData["dead"]) == 1 then
		...
		--SEHIR // FORCE
		setElementDataEx(client, "sehir", tostring(characterData["sehir_furkan"]), true)
		if (tostring(characterData["sehir_furkan"]) == "Secilmedi") then
			triggerClientEvent(client, "sehirGUI", client, tostring(characterData["sehir_furkan"]))
		end
		if mailadresi == "Tanimlanmadi" then
			--outputChatBox(mailadresi, source)
			--outputChatBox(mailadresi, client)
			triggerClientEvent(client, "account:mailekrani", client)
		end
		local motd = getElementData(getRootElement(), "account:motd") or "SalvoMTA'a Hoşgeldiniz | Giriş Paranız Envanterinizdedir |"
		outputChatBox("MOTD: " .. motd, source, 255, 255, 0)
		
		if ((getElementData(source, "adminlevel") or 0) > 0) then
			local amotd = getElementData(getRootElement(), "account:amotd") or "Bütün Adminler Rapor Bakmakla Hükümlüdür"
		...
		triggerEvent("hayvan:createPet",client)
		triggerEvent("social:character", client)
		triggerEvent("oyuncuVipKontrol", client)
...
function Characters_onCharacterChange()
	local karakterid = getElementData(client, "dbid")
	triggerEvent("Account:KarakterSecme",client,karakterid) -- Pet İçin
