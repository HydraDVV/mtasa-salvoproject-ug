...
function newCharacter_create(characterName, race, gender, skin, height, weight, age, languageselected, tip)
	...
	local id = mysql:query_insert_free("INSERT INTO characters SET charactername='" .. characterName .. "',karakter_tip="..mysql:escape_string(tip)..", lastarea='Saint Francis Boulevard', gender='" .. mysql:escape_string(gender) .. "', skincolor='" .. mysql:escape_string(race) .. "', weight='" .. mysql:escape_string(weight) .. "', height='" .. mysql:escape_string(height) .. "', description='', account='" .. mysql:escape_string(accountID) .. "', skin='" .. mysql:escape_string(skin) .. "', age='" .. mysql:escape_string(age) .. "', fingerprint='" .. mysql:escape_string(fingerprint) .. "', lang1=" .. mysql:escape_string(languageselected) .. ", lang1skill=100, currLang=1" )
	...
	if (id) then -- 
		...
		setElementData(client, "karakterlimit", tonumber(getElementData(client, "karakterlimit"))-1)
		exports['anticheat-system']:changeProtectedElementDataEx(client, "dbid")
			
		--if (update) then -- Maxime 4/1/2013
