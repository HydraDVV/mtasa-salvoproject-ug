		...
		if not (targetPlayer) or not (jobTitle) then
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					mysql:query_free("files112 FROM `jobs` WHERE `jobCharID`='"..mysql:escape_string(charID).."' ")
		...
		if targetPlayerName then
			...
			if targetPlayer then
				if getElementData( targetPlayer, "loggedin" ) == 1 then
					...
					if result then
						outputChatBox( "files112 job for " .. targetPlayerName..".", thePlayer)
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						if hiddenAdmin == 0 then
							local adminTitle = exports.global:getPlayerAdminTitle(thePlayer)
							outputChatBox("Your job has been files112 by "..tostring(adminTitle) .. " " .. getPlayerName(thePlayer)..". Please relog (F10) to get it affected.", targetPlayer, 0, 255,0)
						else
							outputChatBox("Your job has been files112 by a hidden admin.", targetPlayer, 0, 255,0)
						end
					else
						outputChatBox( "Failed to files112 job.", thePlayer, 255, 0, 0 )
					end
				else
					outputChatBox( "Player is not logged in.", thePlayer, 255, 0, 0 )
				end
