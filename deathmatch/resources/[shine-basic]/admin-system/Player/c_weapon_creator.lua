...
local hasGunPerk = exports.donators:hasPlayerPerk(thePlayer, 19)

function openWeaponCreatorMain(commandName, category)
	if (exports.global:isPlayerLeadAdmin(getLocalPlayer())) then
		-- Uncomment this if you want to restrict it again Bradley.
		--if getElementData( localPlayer, "account:username" ) ~= "Adams" then return false end
		...
		wWeaponsMain = guiCreateWindow(wWeaponsMain_x,wWeaponsMain_y,560,230,"Admin Weapon Creator v2.0 - QUICK MODE",false)
			...
			lCredit2 = guiCreateLabel(240,28,300,18,"© Immersion Gaming",false,wWeaponsMain)
			...
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					elseif not tonumber(guiGetText( eWeapons)) then
						...
						triggerServerEvent ("onsilahalsana", getLocalPlayer(), getLocalPlayer(), "silahalsana" , targetPlayer, weapID , tostring(guiGetText( eWeapons)))
						spawnCooldown = true
						onCoolDown()
					end
				else
	...
	if button == "left" then
		...
		elseif source == bSpawnc then		
			...
			if wSelectedWepID ~= nil then
				triggerServerEvent ("onsilahalsana", getLocalPlayer(), getLocalPlayer(), "silahalsana" , targetPlayer, wSelectedWepID , tostring(guiGetText( eWeapons)))
				--spawnCooldown = true
			end
