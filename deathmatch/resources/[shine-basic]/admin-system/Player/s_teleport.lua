	...
	if (exports.global:isPlayerAdmin(thePlayer) or exports.global:isPlayerGameMaster(thePlayer)) then
		if commandName:lower() == "goto" then
			if not (target) then
				...
				if targetPlayer then
					...
					if (logged==0) then
						...
						outputChatBox(" #FF0000[!]#FFFFFFOyuncunun yanına ışınlandınız || " .. targetPlayerName .. ".", thePlayer, 0,0,0,true)
						...
						if exports.global:isPlayerGameMaster(thePlayer) then
							...
							if hiddenAdmin == 0 then
								outputChatBox(" #FF0000[!]#FFFFFFAdmin " .. username .. " yanınıza ışınlandı. ", targetPlayer,0,0,0,true)
							end
						end
					end
				end
			...
			if (logged==0) then
				...
				triggerEvent("texture-system:loadCustomTextures", thePlayer)
				if exports.global:isPlayerGameMaster(thePlayer) then
					outputChatBox(" Gamemaster " .. username .. " has accepted your teleport request. ", target)
				else
		...
		if(not from or not to) then
			...
			if(fromplayer and toplayer) then
				...
				if(not logged1 or not logged2) then
					...
					outputChatBox("#FF0000[!]#FFFFFFYanınıza oyuncu ışınladınız. ||" .. targetPlayerName1:gsub("_"," ") .. "" .. targetPlayerName2:gsub("_"," ") .. ".", thePlayer, 0,0,0, true)
					triggerEvent("texture-system:loadCustomTextures", fromplayer)
					local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
					if hiddenAdmin == 0 then
		...
		if not target then
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					outputChatBox("#FF0000[!]#FFFFFFOyuncuyu yanına ışınladın || #FF0000" .. targetPlayerName .. "", thePlayer, 0, 0, 0, true)
					triggerEvent("texture-system:loadCustomTextures", targetPlayer)
					if exports.global:isPlayerFullGameMaster(thePlayer) then
						...
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						if hiddenAdmin == 0 then
							outputChatBox("#FF0000#FFFFFFAdmin " .. username .. " || yanına ışınladı. ", targetPlayer, 0, 0, 0, true)
						end
					end
