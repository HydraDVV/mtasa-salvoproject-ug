...
function CreateCheckWindow()
	...
	Button = {}
	Window = guiCreateWindow(width-400,0,400,385,"Oyuncu İstatistikleri",false)
	...
	Label = {
		...
		guiCreateLabel(0.03,0.22,0.66,0.0887,"donPoints: N/A", true,Window),
...
levels = { "Suspended Admin", "Trial Admin", "Admin", "Super Admin", "Lead Admin", "Head Admin", "Owner" }
levels[-1] = "Trial GameMaster"
levels[-2] = "GameMaster"
levels[-3] = "Super GameMaster"
levels[-4] = "Lead GameMaster"
levels[-5] = "Head GameMaster"
local actions = { [0] = "jail", [1] = "kick", [2] = "ban", [3] = "app", [4] = "warn", [5] = "aban", [6] = "other", [99] = "force-app"}
function OpenCheck( ip, adminreports, donPoints, note, history, warns, transfers, bankmoney, money, adminlevel, hoursPlayed, accountname, hoursAcc )
	...
	guiSetText ( Label[2], "IP: IP Adresleri gizlidir. " )
	guiSetText ( Label[18], "Admin Level: " .. ( levels[adminlevel or 0] or "Player" ) .. " (" .. adminreports .. " Reports)" )
	guiSetText ( Label[11], "Uyarıları: " .. warns )
	if not exports.global:isPlayerAdmin(getLocalPlayer()) then
		guiSetText ( Label[3], "Money: N/A (Bank: N/A)")
	else	
		guiSetText ( Label[3], "Para: $" .. exports.global:formatMoney(money) .. " (Banka Parası: $" .. exports.global:formatMoney(bankmoney) .. ")")
	end
	guiSetText ( Button[5], history )
	guiSetText ( Label[20], "Rp Pointi: " .. donPoints )
	guiSetText ( Label[22], "Stat transfers: " .. transfers )
	guiSetText ( Label[19], "Hours Char: " .. ( hoursPlayed or "N/A" ) .. "\n~ Total: " .. ( hoursAcc or "N/A" ) )
	
	if (player == getLocalPlayer()) and not exports.global:isPlayerLeadAdmin(getLocalPlayer()) then
		guiSetText ( memo, "-You can not view your own note-")
		guiMemoSetReadOnly(memo, true)
		guiSetEnabled(Button[4], false)
	elseif not exports.global:isPlayerAdmin(getLocalPlayer()) then
		guiSetText ( memo, "-You does not have access to admin note-")
		guiMemoSetReadOnly(memo, true)
		guiSetEnabled(Button[4], false)
	else
...
addEventHandler( "onClientRender", getRootElement(),
	function()
		if guiGetVisible(Window) and isElement( player ) then
			local x, y, z = 0, 0, 0
			if not exports.global:isPlayerGameMaster(getLocalPlayer()) and getElementAlpha(player) ~= 0 or exports.global:isPlayerHeadAdmin(getLocalPlayer()) then 
			...
			if team then --and exports.global:isPlayerLeadAdmin(getLocalPlayer()) then -- Maxime | Faction info is visible for Lead+ only
			...
			if vehicle and not exports.global:isPlayerGameMaster(getLocalPlayer()) then
				guiSetText ( Label[10], "Bindiği Araç: " .. getVehicleName( vehicle ) .. " (" ..getElementData( vehicle, "dbid" ) .. ")" )
			else
				guiSetText ( Label[10], "Vehicle: Araçta Değil!")
			end
			if not exports.global:isPlayerGameMaster(getLocalPlayer()) then
				guiSetText ( Label[12], "Mevcut Lokasyonu: " .. getZoneName( x, y, z ) )
