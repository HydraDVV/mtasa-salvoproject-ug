...
BankClass = {
    peds = {
        {1111.0380859375, -776.4501953125, 977.02734375, 147, 5, 90}
			...
			BankClass.timer = Timer(
				function()
					if isElement(BankClass.BankBrowser) and BankReceiver then
						BankReceiver:executeJavascript("change('curbalance', '"..exports.global:formatMoney(localPlayer:getData("bankmoney")).."')");
...
addEventHandler('BankBrowser-DepositMoney', root, function(amount) if exports.global:hasMoney(localPlayer, tonumber(amount)) then triggerServerEvent('BankBrowser-DepositMoney', localPlayer, tonumber(amount)) BankReceiver:executeJavascript("change('curbalance', '"..(localPlayer:getData("bankmoney")+tonumber(amount)).."')"); else outputChatBox("Titan:#ffffff Yeterli paranız olmadığı için parayı kabul edemiyoruz.", 100, 100, 100, true) end end)
addEventHandler('BankBrowser-WidthrawMoney', root, function(amount) local amount = tonumber(amount) if localPlayer:getData('bankmoney') >= amount then triggerServerEvent('BankBrowser-WidthrawMoney', localPlayer, tonumber(amount)) else outputChatBox("Titan:#ffffff Hesabınızda yeterli para yok.", 100, 100, 100, true) end end)
