		...
		if v[1] == infotable[1] and v[2] == infotable[2] then
			for i2,v2 in ipairs(textsToDraw) do
				if v2[1] == v[1] and v[3] - v2[3] == 1 then
					setTimer(removeAme,showtime + (#v[2] * characteraddition),1,v2)
				end
			end
...
function displayAme()
	for i,infotable in ipairs(textsToDraw) do
		...
		local posx,posy = getScreenFromWorldPosition(x,y,z+0.020*distance+0.20)
		...
		if posx and distance <= 45 and isLineOfSightClear(cx,cy,cz,px,py,pz,true,true,false,true,false,true,true, blocking) then -- change this when multiple ignored elements can be specified
			...
			--dxDrawRectangle(posx - (3 + (0.5 * width)),posy - (2 + (infotable[3] * fontHeight)),width + 5,19,tocolor(0,0,0,150))
			--dxDrawRectangle(posx - (3 + (0.5 * width)),posy - (2 + (infotable[3] * fontHeight))+19,width + 5,1,tocolor(unpack(color1)))
