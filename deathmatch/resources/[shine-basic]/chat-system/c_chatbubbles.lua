...
local customFont = dxCreateFont("roboto.ttf", 9)
...
addEventHandler("addChatBubblee", root,
	function(message, command)
		if source ~= localPlayer or selfVisible then
			if command == "ado" or command == "ame" then
				addText(source, message, {196,68,169, 255 }, "default-bold", false, command)
			else
				addText(source, message, {  223, 174, 255,230  })
			end
		end
	end
)
...
addEventHandler("addChatBubbleee", root,
	function(message, command)
		if source ~= localPlayer or selfVisible then
			if command == "ado" or command == "ame" then
				addText(source, message, { 255, 255, 255 }, "default-bold", false, command)
			else
			
				addText(source, message, {132, 203, 173, 230})
			end
		end
	end
)
					...
					if tick < v.expires or v.sticky then
						...
						if getDistanceBetweenPoints3D(x, y, z, px, py, pz) < distanceVisible and isLineOfSightClear ( x, y, z, px, py, pz, true, not isPedInVehicle(player), false, true) and pdim == dim and pint == int then
							...
							if sx and sy then
								...
								local yPos = outElastic(elapsedTime, v.yPos + 130, (sy - fontHeight*i ) - v.yPos - 10, 1000, 5, 500)
								...
								--dxDrawText(v.text, (sx) - (0.5 * width), yPos, sx - (0.5 * width), yPos - (i * fontHeight), tocolor(0,0,0,255) , 1.3, "default-bold", "left", "top", false, false, false)
								dxDrawText(v.text, (sx) - (0.5 * width), yPos, sx - (0.5 * width), yPos - (i * fontHeight), tocolor(unpack(v.color)), 1, exports['titan_fonts']:getFont('Roboto',11), "left", "top", false, false, false)
							end
						end
					else
