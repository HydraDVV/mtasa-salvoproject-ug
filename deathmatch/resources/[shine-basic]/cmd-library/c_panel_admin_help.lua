...
function adminhelp (commands, scriptUpdates)
	...
	myadminWindow = guiCreateWindow ( 0.15, 0.15, 0.7, 0.7,  "Index of admin commands | Dynamic Commands Library by Maxime - UG", true)
	...
	if not exports.global:isPlayerHeadAdmin(getLocalPlayer()) and not getElementData(getLocalPlayer(), "hasCmdLibraryAccess") then
	...
	if exports.global:isPlayerHeadAdmin(getLocalPlayer()) or getElementData(getLocalPlayer(), "hasCmdLibraryAccess") then
...
function copyToClipboard1(button,state)
	...
	if (row==-1) or (col==-1) then
		...
		elseif (exports.global:isPlayerHeadAdmin(getLocalPlayer()) or getElementData(getLocalPlayer(), "hasCmdLibraryAccess")) and (button == "right") and (state == "up") then
...
local function getAdminLevelFromTitle(text)
	...
	elseif text == "Trial Admin" then
		return "1"
	elseif text == "Regular Admin" then
		return "2"
	elseif text == "Super Admin" then
		return "3"
...
function drawCmdModifyWindow(cmdID, CmdName, cmdEx, AccessLevel)
	...
	local eAccess = guiCreateComboBox(113,86,216,24,AccessLevel,false,wCmdModify)
		...
		guiComboBoxAddItem(eAccess, "Super Admin")
		guiComboBoxAddItem(eAccess, "Lead Admin")
		guiComboBoxAddItem(eAccess, "Head Admin")
