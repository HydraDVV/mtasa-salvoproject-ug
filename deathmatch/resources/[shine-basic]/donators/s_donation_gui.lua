...
function bindKeys()
	...
	for k, arrayPlayer in ipairs(players) do
		...
		if not(isKeyBound(arrayPlayer, "", "down", showDonationGUI)) then
			bindKey(arrayPlayer, "", "down", showDonationGUI)
		end
	end
end

function bindKeysOnJoin()
	bindKey(source, "", "down", showDonationGUI)
...
function showDonationGUI(source)
	...
	if (logged==1) then
		...
		if (menuVisible == false or menuVisible == nil) then
			...
			triggerClientEvent(source, "donation-system:GUI:open", source, obtainedTable, availableTable, donPoints)			
		end
	end
end
