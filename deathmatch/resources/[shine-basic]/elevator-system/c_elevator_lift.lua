	...
	if button == "f" and state == true then
		if boundKey and boundKeyElement then
			if isElement(boundKeyElement) then
				...
				outputDebugString("dev_elevators/c_elevator_lift: onClientKey: "..tostring(button))
			end
		end
	end
end)
...
function triggerLiftGUI(key, keyState, pickup)
	--outputDebugString("dev_elevators/c_elevator_lift: triggerLiftGUI")
	if(keyState == "down" and not wElevator) then
		--outputDebugString("dev_elevators/c_elevator_lift: triggerLiftGUI is OK")
		local px,py,pz = getElementPosition(getLocalPlayer())
