...
function reloadOneLift(elevatorID, skipcheck)
	...
	if (dbid > 0 or skipcheck)then
		...
		if not realElevatorElement then
			outputDebugString("dev_elevators/s_elevator_lift.lua: [reloadOneLift] Can't find element")
		end
		...
		outputDebugString("dev_elevators/s_elevator_lift.lua: Tried to reload elevator without ID.")
	end
end
...
function loadOneLift(elevatorID, hasCoroutine)
	...
	if result then
		while true do
		    ...
		    local pickup = createMarker (row.x, row.y, row.z, 3, "cylinder", 1.5, 255, 255,255, 255 )	
			--local pickup = createPickup(row.x, row.y, row.z, 3,  1318)
			--outputDebugString("pickup "..tostring(pickup))
		end
		mysql:free_result(result)
	else
		outputDebugString("dev_elevators/s_elevator_lift.lua: loadOneLift failed")
	end
end
	...
	if result then
		...
		if result then
			...
			if result2 then
				...
				outputDebugString("dev_elevators/s_elevator_lift.lua: loadAllLifts failed")
			end
		else
			outputDebugString("dev_elevators/s_elevator_lift.lua: loadAllLifts failed")
		end
