...
function createStretcher( playerElement, vehicle )
	...
	if hasPlayerStretcherSpawned( playerElement ) then
		if vehicle then
			exports.global:sendLocalMeAction(playerElement, tostring(exports.global:getVehicleName(vehicle)).."'den sedyeyi alır.")
		else
			exports.global:sendLocalMeAction(playerElement, "sedyeyi alır..")
		end
		destroyStretcher( playerElement, vehicle )
	else
		if vehicle then
			exports.global:sendLocalMeAction(playerElement, tostring(exports.global:getVehicleName(vehicle)).."'ye sedyeyi koyar.")
		else
			exports.global:sendLocalMeAction(playerElement, "sedyeyi alır.")
		end
...
local yaralanmaKordinat = createColSphere(1177.4150390625, -1323.580078125, 14.069562911987, 2)


function yaralanma(player)
	if (getElementData(player,'yaralanma') or 0 ) == 1 then
	dbExec(mysql:getConnection(), "UPDATE characters SET yaralanma=1 WHERE id = "..(getElementData(player, "dbid")))
end
end
addEvent('db:yaralanma',true)
addEventHandler('yaralanma',root,yaralanma)