gFactionWindow, gMemberGrid, gMOTDLabel, colName, colRank, colWage, colDuty, colLastLogin, --[[colLocation,]] colLeader, colOnline, gButtonKick, gButtonPromote, gButtonDemote, gButtonEditRanks, gButtonEditMOTD, gButtonInvite, gButtonLeader, gButtonQuit, gButtonExit, wConfirmQuit, eNote = nil
...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty)
	if (gFactionWindow==nil) then
		...
		local teamName = getTeamName(theTeam) or ""
		local playerName = getPlayerName(thePlayer)

		gFactionWindow = guiCreateWindow(0.1, 0.25, 0.85, 0.525, tostring(teamName) or "", true)
		if factionTheTeam == "Yok" then
		loadingLabel = guiCreateLabel(0, 0, 1, 1, "Herhangi bir birliğiniz bulunmuyor!", true, gFactionWindow)
		...
		tabOverview = guiCreateTab("Genel", tabs)
		local birlikOnayi = getElementData (theTeam, "onay") or 0
		local birlikLevel = getElementData (theTeam, "level") or 0
		if birlikOnayi == 0 then
		    birlikOnayi= "Yok"
		elseif birlikOnayi == 1 then
		    birlikOnayi= "Var"
		end	
		birlikOnayi = guiCreateLabel(0.825, 0.00, 0.16, 0.09, "Birlik Onayı: "..birlikOnayi.."", true, tabOverview)
		birlikLevel = guiCreateLabel(0.825, 0.035, 0.16, 0.09, "Birlik Leveli: "..birlikLevel.."", true, tabOverview)
		-- Make members list
		...
		colLastLogin = guiGridListAddColumn(gMemberGrid, "Girilen Gün", 0.13)
		...
		if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) then
			...
			colWage = guiGridListAddColumn(gMemberGrid, "Maaş TL", 0.2)
		--else
		...
		if factionPackages and #factionPackages > 0 then
			colDuty = guiGridListAddColumn(gMemberGrid, "Görev Kıyafeti", 0.06)
		end
		...
		for k, v in ipairs(memberUsernames) do
			...
			if (memberOnline[k]) then
				...
				guiGridListSetItemText(gMemberGrid, row, colOnline, "Pasif", false, false)
		...
		local birlikLevel = getElementData (theTeam, "level") or 0
		local eklenecek = 5*birlikLevel
		local oyuncumik = (counterOnline+counterOffline)
		local sinir = eklenecek + 10
		local nameekle = ""
		if not ((factionType==2) or (factionType==3) or (factionType==4) or (factionType==6)) then
			nameekle = "- Üye Sınırı: "..sinir
		end
		guiSetText(gFactionWindow, tostring(teamName) .. " (" .. counterOnline .. "/" .. (counterOnline+counterOffline) .. " Üye Aktif) "..nameekle)
		-- Make the buttons
		if (localPlayerIsLeader) then
			if not ((factionType==2) or (factionType==3) or (factionType==4) or (factionType==6)) then
				gButtonLevel = guiCreateButton(0.925, 0.001, 0.06, 0.06, "Level Yükselt", true, tabOverview)
			end
			gButtonKick = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Factiondan At", true, tabOverview)
			gButtonLeader = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Liderlik Ver", true, tabOverview)
			gButtonPromote = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Rütbe Al/Ver", true, tabOverview)
			
			if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) then
				gButtonEditRanks = guiCreateButton(0.825, 0.3058, 0.16, 0.06, "Edit Ranks and Wages", true, tabOverview)
			else
				gButtonEditRanks = guiCreateButton(0.825, 0.3058, 0.16, 0.06, "Rütbeleri Ayarla", true, tabOverview)
			end
			
			gButtonEditMOTD = guiCreateButton(0.825, 0.3824, 0.16, 0.06, "Giriş Mesajı", true, tabOverview)
			if oyuncumik<sinir or (factionType==2) or (factionType==3) or (factionType==4) or (factionType==6) then 
				gButtonInvite = guiCreateButton(0.825, 0.459, 0.16, 0.06, "Uye Ekle", true, tabOverview)
			end
			gButtonRespawnui = guiCreateButton(0.825, 0.5356, 0.16, 0.06, "Araçları Yenile", true, tabOverview)
			
			local factionID = getElementData(getLocalPlayer(), "faction")
			local factionPackages = exports.duty:getFactionPackages(factionID)
			--outputDebugString(tostring(factionPackages))
			if factionPackages and #factionPackages > 0 then
				gButtonPerk = guiCreateButton(0.825, 0.6122, 0.16, 0.06, "Duty Ayarları", true, tabOverview)
				addEventHandler("onClientGUIClick", gButtonPerk, btButtonPerk, false)
			end
			
			addEventHandler("onClientGUIClick", gButtonLevel, btLevelEkle, false)
			...
			colVehPlates = guiGridListAddColumn(gVehicleGrid, "Plaka", 0.115)
			colVehLocation = guiGridListAddColumn(gVehicleGrid, "Mekan", 0.17)
			gButtonVehRespawn = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Aracı Yenile", true, tabVehicles)
			gButtonAllVehRespawn = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Tum Araçları Yenile", true, tabVehicles)
			...
			gButtonExit = guiCreateButton(0.825, 0.86, 0.16, 0.06, "Menüyü Kapat", true, tabOverview)
...
function showrespawn()
	...
	showrespawnUI = guiCreateWindow(sx/2 - 150,sy/2 - 50,300,100,"Aracları Yenile", false)
	local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Aracları Yenilemek İstediginizden Eminmisiniz?",true,showrespawnUI)
...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		bRanksClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Cýkýþ", true, wRanks)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("Gecersiz Rütbe Ýsmi!", 255, 0, 0)
		elseif not (isNumber) then
			outputChatBox("Sayý Ve Sembol Kullanmayýnýz.", 255, 0, 0)
		else
			local sendRanks = { }
			local sendWages = { }
...
function btEditMOTD(button, state)
	if (source==gButtonEditMOTD) and (button=="left") and (state=="up") then
		if not (wMOTD) then
			...
			wMOTD = guiCreateWindow(x, y, width, height, "Gunluk Mesaj", false)
			tMOTD = guiCreateEdit(0.1, 0.2, 0.85, 0.1, tostring(theMotd), true, wMOTD)
			
			guiSetInputEnabled(true)
			
			bUpdate = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Yukselt", true, wMOTD)
...
function sendMOTD(button, state)
	if (source==bUpdate) and (button=="left") and (state=="up") then
		...
		if (found1) or (found2) then
			outputChatBox("Mesajýnda Gecersiz Karakterler Var.", 255, 0, 0)
		else
...
function btInvitePlayer(button, state)
	if (source==gButtonInvite) and (button=="left") and (state=="up") then
		if not (wInvite) then
			...
			wInvite = guiCreateWindow(x, y, width, height, "Uye Al", false)
			tInvite = guiCreateEdit(0.1, 0.2, 0.85, 0.1, "Uye Ýsmi", true, wInvite)
			addEventHandler("onClientGUIChanged", tInvite, checkNameExists)
					
			lNameCheck = guiCreateLabel(0.1, 0.325, 0.8, 0.3, "Oyuncu Bulunamadý!", true, wInvite)
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		--else
			...
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Ayrýlmak Ýstediginizden Eminmisiniz?" .. getTeamName(theTeam) .. "?",true,wConfirmQuit)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
			local bYes = guiCreateButton(0.1,0.65,0.37,0.23,"Evet",true,wConfirmQuit)
			local bNo = guiCreateButton(0.53,0.65,0.37,0.23,"Hayýr",true,wConfirmQuit)
...
function btLevelEkle(button, state)
	if (button=="left") and (state=="up") and (source==gButtonLevel) then
		local oyuncu_birlik = getPlayerTeam(getLocalPlayer())
		local birlikTip = getElementData(oyuncu_birlik, "type")
		local oyuncu_birlik_isim = getTeamName(oyuncu_birlik)
		local birlik_seviye = getElementData(oyuncu_birlik, "level") or 1
		local eklenecek = 5*birlik_seviye
		local sinir = eklenecek + 15
		triggerEvent("birlikSeviyeGUI", getLocalPlayer(), oyuncu_birlik_isim, tostring(birlik_seviye), sinir)
	end
end
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		--[[else]]if (playerName~="") then
			...
			outputChatBox("Factiondan Atıldı.")
		end
	end
end
