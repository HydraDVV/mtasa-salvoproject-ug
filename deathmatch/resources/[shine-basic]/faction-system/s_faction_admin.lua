...
function adminShowFactions(thePlayer)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		local result = mysql:query("SELECT id, name, type, onay, (SELECT COUNT(*) FROM characters c WHERE c.faction_id = f.id) AS members FROM factions f ORDER BY id ASC")
		if result then
			local factions = { }
			...
			while true do
				...
				table.insert( factions, { row.id, row.name, row.type, ( getTeamFromName( row.name ) and #getPlayersInTeam( getTeamFromName( row.name ) ) or "?" ) .. " / " .. row.members, onay_str } )
			end
...
function birlikOnayla(thePlayer, commandName, factionID, onay)
	if (exports.global:isPlayerHeadAdmin(thePlayer)) then
		if not (factionID) or not (onay)  then
			outputChatBox("SYNTAX: /" .. commandName .. " [Birlik ID] [1 - Var/ 0 - Yok]", thePlayer, 255, 194, 14)
		else
			factionID = tonumber(factionID)
			if factionID and factionID > 0 then
				local theTeam = exports.pool:getElement("team", factionID)
				if (theTeam) then
					mysql:query_free("UPDATE factions SET onay='" .. tonumber(onay) .. "' WHERE id='" .. factionID .. "'")
					local oldName = getTeamName(theTeam)
					setElementData(theTeam, "onay", tonumber(onay))
					outputChatBox(oldName .. " isim li birliğin onay durumu güncellendi("..onay..").", thePlayer, 255, 0, 0)
					--[[if tonumber(onay) == 1 then
						...
						exports.factions:sendNotiToAllFactionMembers(factionID, "Birliğiniz '"..oldName.."', "..exports.global:getPlayerFullIdentity(thePlayer,1,true) .. " tarafından onaylanmıştır!")
					elseif tonumber(onay) == 0 then
						...
						exports.factions:sendNotiToAllFactionMembers(factionID, "Birliğiniz '"..oldName.."', "..exports.global:getPlayerFullIdentity(thePlayer,1,true) .. " tarafından onayı kaldırıldı!")
					end]]
