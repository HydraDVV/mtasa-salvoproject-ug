...
function loadAllFactions(res)
	...
	if result then
		while true do
			...
			exports['anticheat-system']:changeProtectedElementDataEx(theTeam, "onay", onay, true)
			exports['anticheat-system']:changeProtectedElementDataEx(theTeam, "level", level, true)
			...
			exports['anticheat-system']:changeProtectedElementDataEx(theTeam, "fnote", row.fnote == mysql_null() and "" or row.fnote, false)
		...
		for k, thePlayer in ipairs(players) do
			...
			if result then
				...
				exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "factionPackages", fromJSON(tostring(result.faction_perks)) or { }, false)
...
function getPlayerFaction(playerName)
	...
	if (not thePlayerElement or override) then  -- Player is offline
		...
		if row then
			return 1, tonumber(row["faction_id"]), tonumber(row["faction_rank"]), tonumber(row["faction_leader"]), type(row["faction_perks"]) == "string" and (fromJSON(row["faction_perks"]) or { }), nil
		end
	end
			...
			if (factionID~=-1) then
				...
				if query then
					...
					while true do
						...
						memberPerks[i] = fromJSON(tostring(row.faction_perks)) or { }
					...
					if hasPlayerAccessOverFaction(source, factionID) then
						local result = mysql:query("SELECT id, model, currx, curry, currz, plate FROM vehicles WHERE faction=" .. factionID .. "")
						if result then
							local j = 1
							while true do
								local row = mysql:fetch_assoc(result)
								if not row then break end
								vehicleIDs[j] = row.id
								vehicleModels[j] = getVehicleNameFromModel(tonumber(row.model))
					...
					local theTeam = getPlayerTeam(source)
					triggerClientEvent(source, "showFactionMenu", getRootElement(), motd, memberUsernames, memberRanks, hasPlayerAccessOverFaction(source, factionID) and memberPerks or {}, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks,  factionWages, theTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty)
				end
			else
...
function callbackUpdateRanks(ranks, wages)
	...
	if (wages) then
		for i = 1, 20 do
			wages[i] = math.min(7500, math.max(0, tonumber(wages[i]) or 0))
		end
...
function callbackUpdateFNote(fnote)
	...
	if (factionID~=-1) then
		if mysql:query_free("UPDATE factions SET fnote='" .. tostring(mysql:escape_string(fnote)) .. "' WHERE id='" .. factionID .. "'") then
			...
			exports['anticheat-system']:changeProtectedElementDataEx(theTeam, "fnote", fnote, false)
		else
...
function callbackRemovePlayer(removedPlayerName)
	...
	if mysql:query_free("UPDATE characters SET faction_id='-1', faction_leader='0', faction_rank='1', dutyskin = 0, duty = 0 WHERE charactername='" .. mysql:escape_string(removedPlayerName) .. "'") then
		...
		if (removedPlayer) then -- Player is online
			...
			exports['anticheat-system']:changeProtectedElementDataEx(targetPlayer, "factionleader", 0, false)
...
function callbackToggleLeader(playerName, isLeader)
	...
	if (isLeader) then -- Make player a leader
		...
		if mysql:query_free("UPDATE characters SET faction_leader='1' WHERE charactername='" .. mysql:escape_string(playerName) .. "'") then
			...
			local teamPlayers = getPlayersInTeam(theTeam)
			for k, v in ipairs(teamPlayers) do
				outputChatBox(username:gsub("_", " ") .. " promoted " .. playerName:gsub("_", " ") .. " to leader.", v, 255, 194, 14)
			end
		...
		if mysql:query_free("UPDATE characters SET faction_leader='0' WHERE charactername='" .. mysql:escape_string(playerName) .. "'") then
			...
			local teamPlayers = getPlayersInTeam(theTeam)
			for k, v in ipairs(teamPlayers) do
				outputChatBox(username:gsub("_", " ") .. " demoted " .. playerName:gsub("_", " ") .. " to member.", v, 255, 194, 14)
			end
		else
...
function callbackPromotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if mysql:query_free("UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. mysql:escape_string(playerName) .. "'") then
		...
		if (theTeam) then
			local teamPlayers = getPlayersInTeam(theTeam)
			for k, v in ipairs(teamPlayers) do
				outputChatBox(username:gsub("_", " ") .. " promoted " .. playerName:gsub("_"," ") .. " from '" .. oldRank .. "' to '" .. newRank .. "'.", v, 255, 194, 14)
			end
		end
	else
...
function callbackDemotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if mysql:query_free("UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. safename .. "'") then
		...
		if (theTeam) then
			local teamPlayers = getPlayersInTeam(theTeam)
			for k, v in ipairs(teamPlayers) do
				outputChatBox(username:gsub("_", " ") .. " demoted " .. playerName:gsub("_", " ") .. " from '" .. oldRank .. "' to '" .. newRank .. "'.", v, 255, 194, 14)
			end
		end
	else
...
function callbackQuitFaction()
	...
	elseif mysql:query_free("UPDATE characters SET faction_id='-1', faction_leader='0', dutyskin = -1, duty = 0, faction_perks='{}' WHERE charactername='" .. safename .. "'") then
		...
		if theTeam ~= newTeam then
			...
			for k, v in ipairs(teamPlayers) do
				if (v~=thePlayer) then
					outputChatBox(username:gsub("_", " ") .. " has quit the faction '" .. theTeamName .. "'.", v, 255, 194, 14)
				end
			end
		end
	else
...
function callbackInvitePlayer(invitedPlayer)
	...
	local birlik_tip = tonumber(getElementData(theTeam, "type"))
	if (birlik_tip == 2 or birlik_tip == 3 or birlik_tip == 4 or birlik_tip == 6) and getElementData(invitedPlayer, "karakter_tip") == 1 then
		outputChatBox("Eklemek istediğiniz kişi illegal olduğu için legal bir birliğe davet edilemez.", client, 255, 0, 0)
		return
	end
	if (birlik_tip == 0 or birlik_tip == 1 or birlik_tip == 5 or birlik_tip == 8) and getElementData(invitedPlayer, "karakter_tip") == 0 then
		outputChatBox("Eklemek istediğiniz kişi legal olduğu için illegal bir birliğe davet edilemez.", client, 255, 0, 0)
		return
	end
