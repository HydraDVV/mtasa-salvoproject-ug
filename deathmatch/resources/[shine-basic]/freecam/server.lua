		...
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				for k, v in ipairs( getElementsByType( "player" ) ) do
					...
					if getElementData(v, "faction") == 20 then
						outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Bağışlar: $" .. exports.global:formatMoney(earnings), v, 200, 100, 200)
					end
				end
				
				exports.logs:dbLog(player, 23, player, "TRT STOP - VIEWERS: " .. watching .. " EARNINGS: " .. earnings)
			else
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Kazanç: $" .. exports.global:formatMoney(earnings), player, 200, 100, 200)
			else
...
function add( affectedPlayers )
	...
	if isTVRunning() and shownto > 0 then
		...
		exports.global:giveMoney(getTeamFromName"TRT Haber Bülteni", LSN_MONEY * shownto)
