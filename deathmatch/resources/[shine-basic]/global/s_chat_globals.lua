...
function changeWarnStyle(thePlayer, commandName)
	if exports.global:isPlayerAdmin(thePlayer) then
		...
		if query then
			exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "wrn:style", warnStyle)
...
function sendMessageToAdmins(message,showToOffDutyAdmins)
	...
	for k, thePlayer in ipairs(players) do
		if (exports.global:isPlayerAdmin(thePlayer)) then
			if showToOffDutyAdmins or (getElementData(thePlayer, "adminduty") == 1) then
...
--MAXIME
function sendMessageToGameMasters(message, showToOffDutyGMs)
	local players = exports.pool:getPoolElementsByType("player")
	for k, thePlayer in ipairs(players) do
		if (exports.global:isPlayerGameMaster(thePlayer)) then
			if showToOffDutyGMs or getElementData(thePlayer, "account:gmduty") then
				if getElementData(thePlayer, "report-system:isAdminPanelEnabled") then
					exports["report-system"]:showToAdminPanel(message, thePlayer, 255,0,0)
				else
					if getElementData(thePlayer, "wrn:style") == 1 then
...
function findPlayerByPartialNick(thePlayer, partialNick, fromBankSystem)
	...
	local partialNick = string.lower(partialNick) or thePlayer
...
function sendLocalText(root, message, r, g, b, distance, exclude, useFocusColors)
	...
	if getElementType(root) == "player" and exports['freecam']:isPlayerFreecamEnabled(root) then return end
	...
	if getElementType(root) == "player" and shownto > 0 and getElementDimension(root) == 127 then -- TV SHOW!
		exports['freecam']:add(shownto, message)
	end
