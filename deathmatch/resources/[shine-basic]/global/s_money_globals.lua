...
function takeBankMoney(thePlayer, amount, rest)
	amount = tonumber( amount ) or 0
	if amount == 0 then
		return true, 0
	elseif thePlayer and isElement(thePlayer) and amount > 0 then
		...
		local money = getElementData( thePlayer, "bankmoney" ) or 0
		if rest and amount > money then
			amount = money
		end
		...
		if amount == 0 then
			return true, 0
		elseif hasBankMoney(thePlayer, amount) then
			exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "bankmoney", money - amount, false )
			if getElementType(thePlayer) == "player" then
				mysql:query_free("UPDATE characters SET bankmoney = bankmoney - " .. amount .. " WHERE id = " .. getElementData( thePlayer, "dbid" ) )
				-- Money
				for i = 134, 134 do
					--while exports['item-system']:takeItem(thePlayer, i) do
					--end
...
function hasBankMoney(thePlayer, amount)
	...
	if thePlayer and isElement(thePlayer) and amount >= 0 then
		...
		return getBankMoney(thePlayer) >= amount
	end
	return false
end
...
function getBankMoney(thePlayer, nocheck)
	if not nocheck then
		checkMoneyHacks(thePlayer)
	end
	return getElementData(thePlayer, "bankmoney") or 0
end
