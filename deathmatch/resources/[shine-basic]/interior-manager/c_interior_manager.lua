...
function createIntManagerWindow(IntsList)
	...
	if (IntsList) then
		...
		--IntsList1 = IntsList
		gWin = guiCreateWindow(sx/2-400,sy/2-300,800,600,"Interiors Manager v1.0 © Maxime - UnitedGaming",false)
		...
		bDelInt = guiCreateButton(0.0113,0.035,0.0938,0.045,"DELETE INT",true,gWin)
			guiSetFont(bDelInt,"default-bold-small")
		bRestoreInt = guiCreateButton(0.0113,0.085,0.0938,0.045,"RESTORE INT",true,gWin)
		...
		bGotoInt = guiCreateButton(0.1175,0.085,0.1,0.045,"GO TO INT",true,gWin)
		...
		bRemoveInt = guiCreateButton(0.23,0.085,0.1,0.045,"REMOVE INT",true,gWin)
...
function createCheckIntWindow(result, adminTitle, history)
	...
	guiSetText(lOwner, "Owner: "..charName(result[1][5])..cked(result[1][7])..accountName(result[1][6]))
	...
	guiSetText(mAdminNote, result[1][16] or "\n")
	-- Fetching history
	if history then
