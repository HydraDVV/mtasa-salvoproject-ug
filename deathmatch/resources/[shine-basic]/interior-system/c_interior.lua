...
timer = nil
intNameFont = guiCreateFont( "components/intNameFont.ttf",19)
-- Message on enter
function showIntName(name, ownerName, inttype, cost, ID, bizMsg)
	...
	elseif name then
		...
		if (ownerName=="None") and (inttype==3) then -- Unowned type 3 (rentable)
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "F'ye basarak alabilirsiniz." .. tostring(exports.global:formatMoney(cost)) .. "TL.", true)
		...
		elseif (ownerName=="None") and (inttype<2) then -- Unowned any other type
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "F'ye basrak alabilirsiniz. " .. tostring(exports.global:formatMoney(cost)) .. "TL.", true)
			guiSetFont(gBuyMessage, "default")
			guiLabelSetHorizontalAlign(gBuyMessage, "center", true)
			--guiSetAlpha(gBuyMessage, 0.0)
		else
			local msg = "F'ye basarak girebilirsiniz."
...
addEventHandler("playerKnock", root,
	function()
		...
		playSound("components/knocking.mp3")
	end
)
...
addEventHandler("doorUnlockSound", root,
	function()
		--outputChatBox(" * Knocks can be heard coming from the door. *      ((" .. getPlayerName(source):gsub("_"," ") .. "))", 255, 51, 102)
		playSound("components/doorUnlockSound.mp3")
	end
)
...
addEventHandler("doorLockSound", root,
	function()
		--outputChatBox(" * Knocks can be heard coming from the door. *      ((" .. getPlayerName(source):gsub("_"," ") .. "))", 255, 51, 102)
		playSound("components/doorLockSound.mp3")
	end
)
...
addEventHandler("doorGoThru", root,
	function()
		--outputChatBox(" * Knocks can be heard coming from the door. *      ((" .. getPlayerName(source):gsub("_"," ") .. "))", 255, 51, 102)
		playSound("components/doorGoThru.mp3")
	end
)
