		...
		elseif not targetPlayerName then
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 10 and getElementDimension(targetPlayer) == getElementDimension(thePlayer) then
					if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") or exports.global:isPlayerFullAdmin(thePlayer) or exports.donators:hasPlayerPerk(thePlayer, 14) then
...
function buyInterior(player, pickup, cost, isHouse, isRentable)
	...
	if exports.global:takeMoney(player, cost) then
		...
		outputChatBox("#FF0000[!]#FFFFFFÜzgünüz, bu mülkü satın alamazsınız.", player, 0, 0, 0, true)
	end
end
--[[
		...
		if locked == 0 then
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					...
					exports.global:sendLocalText(source, " * Sağ cebindeki anahtarı alır ve kilite geçirerek saat yönünde çevirir ve açar.*", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "unlock without key", source)
				end
			else
				exports.global:sendLocalMeAction(source, "* Sağ cebindeki anahtarı alır ve kilite geçirerek saat yönünde çevirir ve açar..")
			end
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					...
					exports.global:sendLocalText(source, "* Cebindeki anahtarı alır ve kilite sokarak saat yönünde çevirir ve kapıyı kitler.", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "lock without key", source)
				end
			else 
				exports.global:sendLocalMeAction(source, "* Cebindeki anahtarı alır ve kilite sokarak saat yönünde çevirir ve kapıyı kitler..")
			end
...
addEventHandler("onPlayerInteriorChange", getRootElement( ),
	function( a, b, toDimension, toInterior)	
		...
		triggerEvent("texture-system:loadCustomTextures", source)
	end
)
