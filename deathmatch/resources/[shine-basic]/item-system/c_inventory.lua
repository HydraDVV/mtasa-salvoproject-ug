...
local background_color = tocolor( 0, 0, 0, 230 )
...
local empty_color = tocolor( 0, 0, 0, 200 )
local full_color = tocolor( 0, 0, 0, 200 )
local tooltip_text_color = tocolor( 255, 255, 255, 255 )
local tooltip_background_color = tocolor ( 0, 0, 0, 200 )
...
local actionIcons =
{
	[TAB_WALLET] = { -48, tocolor( 0, 0, 0, 50 ), "Cüzdan" },
	[TAB_ITEMS] = { 48, tocolor( 0, 0, 0, 50 ), "Çanta" },
	[TAB_KEYS] = { 4, tocolor( 0, 0, 0, 50 ), "Anahtarlar" },
	[TAB_WEAPONS] = { -28, tocolor( 0, 0, 0, 50 ), "Silahlar" },
	
	[ACTION_DROP] = { -202, tocolor( 127, 255, 127, 63 ), "Esya Birak", "Press CTRL while selecting an item to automatically drop it." },
	[ACTION_SHOW] = { -201, tocolor( 127, 127, 255, 63 ), "Esya Goster" },
	[ACTION_DESTROY] = { -200, tocolor( 255, 127, 127, 63 ), "Esya Yoket", "Press DELETE while selecting an item to automatically delete it." },
}
...
local function tooltip( x, y, text, text2 )
	...
	roundedRectangle( x, y, width, height, tooltip_background_color )
	dxDrawText( text, x, y, x + width, y + height, tooltip_text_color, 1, hfont, "center", "center", false, false, true )
end
...
local function getTooltipFor( itemID, itemValue, isGrouped )
	...
	elseif itemID == 150 then -- ATM card / MAXIME
		name = name.." (Card Number: "..itemValue:sub( itemValue:find( ";" ) + 1 )..")"
	else
		if itemValue and itemValue ~= 1 and #tostring(itemValue) > 0 and not g_items[itemID][2]:find("#v") and itemValue ~= itemName then -- if #v is there, it's put into the description already
			name = name .. " (" .. itemValue .. ")"
		end
	...
	function( )
		...
		elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) then
			...
			if show then
				...
				local y2 = y + sbox
				roundedRectangle( x2, y2, sbox, ( jrows - irows + 1 ) * sbox + spacer, background_color, background_color )
				for i = irows, jrows do
					local icon = actionIcons[ i ]
					local boxx = x2 + spacer
					local boxy = y2 + spacer + sbox * ( i - irows )
					roundedRectangle( boxx, boxy, box, box, i == activeTab and active_tab_color or icon[2],i == activeTab and active_tab_color or icon[2] )
				...
				-- actual inv
				roundedRectangle( x, y, columns * sbox + spacer, rows * sbox + spacer, background_color, background_color )
				for i = 1, columns * 5 do
					...
					if item then
						if not isMove or item[4] ~= clickItemSlot.id then
							roundedRectangle( boxx, boxy, box, box, full_color, full_color )
							...
							if #text > 0 then
								dxDrawText( text, boxx + 2, boxy + 2, boxx + box - 2, boxy + box - 2, tooltip_text_color, 1, hfont, "right", "bottom", true, true, true )
							end
						...
						roundedRectangle( boxx, boxy, box, box, empty_color, empty_color )
					end
				end
			end
			...
			if clickDown and ( getTickCount( ) - clickDown >= 200 ) and ( ( clickItemSlot and not clickItemSlot.group ) or clickWorldItem ) then
					...
					if isCursorOverInventory then
						...
						elseif hoverElement then
							...
							elseif getElementModel( hoverElement ) == 2942 and item [ 1 ] == 150 then -- ATM card into ATM/ MAXIME
								color = background_movetoelement_color
							-- elseif getElementType( hoverElement ) == "ped" and  getElementData(hoverElement,"shopkeeper") and item [ 1 ] == 121 then -- Supplies to normal shops.
								-- color = background_movetoelement_color
				...
				roundedRectangle( boxx - spacer, boxy - spacer, box + 2 * spacer, box + 2 * spacer, background_color )
				roundedRectangle( boxx, boxy, box, box, color )
				dxDrawImage( boxx, boxy, box-10, box-10, getImage( item[1], item[2] ) )
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_DESTROY then
								...
								elseif itemID == 134 then --Money
									outputChatBox("Para Yok Edilemez")
								else
									triggerServerEvent( "destroyItem", localPlayer, itemSlot )
								end
...
bindKey( "i", "down",
	function( )
		if show then
			hideNewInventory( )
		elseif not getElementData(localPlayer, "adminjailed") and not getElementData(localPlayer, "pd.jailstation") or exports.global:isPlayerAdmin(localPlayer) then -- Maxime | Allowed admins+ to access inv in jail.
			show = true
			activeTabItem = nil
			inventory = false
			showCursor( true )
