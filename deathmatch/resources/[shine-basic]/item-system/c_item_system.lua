...
function copyClipboard()
	...
	if setClipboard(text) then
		exports.hud:sendBottomNotification(getLocalPlayer(), "Copying", "Copied '" .. text .. "' to clipboard.")
	else
	...
	if itemSlot > 0 then -- ITEMS
		...
		elseif (itemID==19) then -- MP3 PLayer
			if isPedInVehicle(getLocalPlayer()) or getElementData(getLocalPlayer(), "fishing") or getElementData(getLocalPlayer(), "jammed") then
				exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Use the - and = keys to use the MP3 Player.", 255, 194, 14)
			else
		...
		elseif (itemID==45) or (itemID==46) or (itemID==47) or (itemID==66) then
			exports.hud:sendBottomNotification(source, "Item Usage", "Right click a player to use this item.", source , 255, 0, 0)
			return
		elseif (itemID==50) or (itemID==51) or (itemID==52) then
			hideInventory()
		elseif (itemID==53) then -- Breathalizer
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Use /breathtest to use this item.", 255, 194, 15)
			return
		...
		elseif (itemID==61) then -- Emergency Light Becon
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Put it in your car inventory and press 'P' to toggle it.", 255, 194, 14)
			return
		...
		elseif (itemID==67) then -- GPS
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Put it in your car inventory and Press 'F5'.", 255, 194, 14)
			return
		elseif (itemID==70) then -- First Aid Kit
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Right click on a player who's knocked out to stabilize him.", 255, 194, 14)
			return
		elseif (itemID==71) then -- Notebook
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Use /writenote [text] to write a note. There are " .. itemValue .. " pages left.", 255, 194, 14)
			return
		elseif (itemID==72) then -- Note
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "The Note reads: " .. itemValue, 255, 194, 14)
		elseif (itemID==78) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "This San Andreas Pilot License was issued for " .. itemValue .. ".", 255, 194, 14)
			return
		elseif (itemID==81) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Drop this Fridge in an Interior.", 255, 194, 14)
			return
		elseif (itemID==84) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Put it in a car or carry it to know when police is around.", 255, 194, 14)
			return
		elseif (itemID==85) then -- Emergency Light Becon
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Put it in your car inventory and press 'N' to toggle it.", 255, 194, 14)
			return
		...
		elseif (itemID==103) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Drop this shelf in an interior.", 255, 194, 14 )
			return
		elseif (itemID==111) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage",  "A personal GPS, shiny. Got the latest maps installed.", 255, 0, 0 )
			return
		elseif itemID == 115 or itemID == 116 then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "This item is (part of) a weapon, you cannot use it with clicking on it. If you have all the parts needed, the gun will appear in your scrollmenu.", 255, 194, 14)
			return
		...
		elseif itemID == 117 then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Please place this item into a vehicle, then control it over the vehicles right mouse button menu.", 255, 194, 14)
			return
		elseif itemID == 121 then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "A heavy box full of supplies!", 255, 194, 14)
			return
		...
		elseif (itemID==148) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "OCW permit - " .. itemValue .. ".", 255, 194, 14)
			return
		elseif (itemID==149) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "CCW permit - " .. itemValue .. ".", 255, 194, 14)
			return
		elseif (itemID==150) then -- ATM card
			local cardNumber12 = itemValue:sub( itemValue:find( ";" ) + 1 )
			exports.hud:sendBottomNotification(getLocalPlayer(), g_items[itemID][1],"Card Number: '"..cardNumber12.."' - Owner: '"..tostring(itemValue:sub(1, (itemValue:find( ";" )-1))):gsub("_", " ").."'. Drag & Drop to a nearby ATM Machine.", 255, 194, 14)
			if setClipboard(cardNumber12) then
				outputChatBox( "Copied '"..cardNumber12.."' to clipboard.")
			end
			return
		end
		...
		if itemSlot == -100 then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "You wear Body Armor.", 0, 255, 0)
		else
