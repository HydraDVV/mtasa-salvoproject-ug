			...
			if item then
				...
				if itemID < 0 and itemID ~= -100 then -- weapon
					...
					elseif free == 0 and cap == 0 then
						local weaponName = "Bilinmeyen Silah"
					...
					elseif free == 0 then
						outputChatBox( "|| Aspection Roleplay || Bu silahi silemediniz.", 255, 0, 0 )
					else
						guiSetVisible( wWait, true )
						guiSetEnabled( wInventory, false )
...
local function openElementInventory( ax, ay )
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		wInventory = guiCreateWindow(ax, ay, 400, 300, type .. " Envanter", false)
		...
		UIColName = guiGridListAddColumn(gUserItems, "Isim", 0.9)
		
		gElementItems = guiCreateGridList(0.5, 0.15, 0.45, 0.65, true, wInventory)
		VIColName = guiGridListAddColumn(gElementItems, "Isim", 0.9)
		...
		bGiveItem = guiCreateButton(0.05, 0.81, 0.45, 0.075, "Aktar ---->", true, wInventory)
		...
		bTakeItem = guiCreateButton(0.5, 0.81, 0.45, 0.075, "<---- Aktar ", true, wInventory)
		...
		outputChatBox("|| Aspection Roleplay || Lutfen bekleyiniz.", 255, 0, 0)
	end
end
