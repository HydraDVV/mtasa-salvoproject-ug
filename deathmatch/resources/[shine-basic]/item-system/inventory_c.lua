		...
		elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) then
			...
			if show then
				...
				for i = irows, jrows do
					...
					if i == activeTab then
						--dxDrawRectangle(boxx,boxy,box,box,0,0,0,200,5,true)
					else
						--dxDrawRectangle(boxx,boxy,box,box,0,0,0,200,1,true)
					end
					dxDrawRectangle(boxx,boxy+box-2,box,2,tocolor(220, 225, 225))
			...
			elseif not tooltipYet and getPedOccupiedVehicle(localPlayer) then
				...
				if element then
					...
					if not any and getElementType(element) == "vehicle" and element ~= getPedOccupiedVehicle( localPlayer ) and exports['titan_vehicles']:hasVehiclePlates( element ) and getDistanceBetweenPoints3D( px, py, pz, getElementPosition( element ) ) < 10 then
					...
					if state == "up" and clickItemSlot then
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_SPLIT then
								...
								elseif itemID == 115 then
									if exports.titan_integration:isPlayerTrialAdmin(localPlayer) then -- restrict this to trial admin+ / maxime
										renameWeapon(item)
									else
				...
				elseif button == "left" and isCursorOverInventory and hoverAction and state == "down" then
					if show then
						...
						elseif activeTab == hoverAction then
							show = false
							showCursor( false )
							--exports["titan_realism"]:showSpeedo()
						else
							activeTab = hoverAction
						end
					else
	...
	function( )
		if getElementData(localPlayer, "loggedin") == 1 then
			...
			elseif ((not getElementData(localPlayer, "adminjailed")) or exports.titan_integration:isPlayerTrialAdmin(localPlayer)) and getElementData(getLocalPlayer(), "viewingInterior")~=1 then -- Chaos | Allowed admins to view inventory in jail, not in previewing interiors
				...
				inventory = false
				showCursor( true )
				--exports["titan_realism"]:hideSpeedo()
				playSoundInvOpen()
