...
function getMaxWeight(element)
	if getElementType( element ) == "player" then
		if hasItem(element, 48) and hasItem(element, 126) then
			return 35
		elseif not hasItem(element, 48) and hasItem(element, 126) or hasItem(element, 48) and not hasItem(element, 126) then
			return 20
		else
			return 15
		end		
	elseif getElementType( element ) == "vehicle" then
		...
		elseif isTruck( element ) then
			return tonumber(exports["job-system-trucker"]:getTruckCapacity(element)) or 70
		else
			return 20
		end
