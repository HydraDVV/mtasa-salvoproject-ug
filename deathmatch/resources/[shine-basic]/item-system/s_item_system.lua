...
function useItem(itemSlot, additional)
	...
	if itemID then
            ...
            for i,v in ipairs ( getElementsByType ( "object" ) ) do
                ...
                if distance < 30 and getElementData ( v, "dbid" ) == tonumber ( itemValue ) and not getElementData ( v, "lift.moving" ) then
                    ...
                    if not getElementData ( v, "lift.up" ) then
                        ...
                        triggerEvent('sendAme',  source, "araba asansörünü yükselten rampa kumandaları üzerindeki düğmeye basar" )
		...
		elseif (itemID==8) then -- sandwich
			...
			exports.global:sendLocalMeAction(source, "..")
			takeItemFromSlot(source, itemSlot)
			elseif (itemID==281) then -- sandwich
			giveHealth(source, 5)
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			exports.global:sendLocalMeAction(source, "elini uzatır ve nargileyi içer.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==16) then -- clothes
			...
			--if fittingskins[gender] and fittingskins[gender][race] and fittingskins[gender][race][skin] then
				...
				exports.global:sendLocalMeAction(source, "sağ ve sol eli ile üstündeki kıyafetleri adım adım değiştirir.")
				--if vehicle then
		...
		elseif (itemID==17) then -- watch
			exports.global:sendLocalMeAction(source, "sol kolundaki saate bakar.")
			outputChatBox("#FF0000[!]#FFFFFFŞuanda saat : " .. string.format("%02d:%02d", getTime()) .. ".", source, 0, 0, 0, true)
		...
		elseif (itemID==20) then -- STANDARD FIGHTING
			...
			outputChatBox("#FF0000[!]#FFFFFFSavunma sanatı hakkında kitap okudunuz.", source, 0, 0, 0, true)
		...
		elseif (itemID==21) then -- BOXING
			...
			outputChatBox("#FF0000[!]#FFFFFFBox eğitimi hakkında kitap okudunuz..", source, 0, 0, 0, true)
		...
		elseif (itemID==22) then -- KUNG FU
			...
			outputChatBox("#FF0000[!]#FFFFFFKung Fu eğitimi hakkında kitap okudunuz.", source, 0, 0, 0, true)
		...
		elseif (itemID==23) then -- KNEE HEAD
			...
			outputChatBox("#FF0000[!]#FFFFFFKitabı açarsınız ve kitabın eski Yunanca yazıldığını görürsünüz..", source, 0, 0, 0, true)
		...
		elseif (itemID==24) then -- GRAB KICK
			...
			outputChatBox("#FF0000[!]#FFFFFFGrab Kick Fighting'in nasıl yapılacağı hakkında bir kitap okudun.", source, 0, 0, 0, true)
		...
		elseif (itemID==25) then -- ELBOWS
			...
			outputChatBox("#FF0000[!]#FFFFFFElbow Fighting'in nasıl yapılacağı hakkında bir kitap okudun.", source, 0, 0, 0, true)
		...
		elseif (itemID==27) then -- FLASHBANG
			...
			exports.global:sendLocalMeAction(source, "flashbang'in pimini çeker ve yere bırakır.")
		...
		elseif (itemID==29) then -- RAM
			--if (getElementData(source, "duty") > 0) and (getElementModel(source) == 285) then
				...
				if not (found) then
					outputChatBox("#FF0000[!]#FFFFFFEtrafında kıracak kapı yok", source, 0, 0, 0, true)
				else
					...
					if (interiorStatus[1] ~= 2) and (interiorStatus[4] < 0) and (interiorStatus[3]) and not (interiorStatus[2]) then
						outputChatBox("#FF0000[!]#FFFFFFBu kapı bir oyuncuya ait değil..", source, 255, 0, 0, true)
					elseif interiorStatus[2] then
						outputChatBox("#FF0000[!]#FFFFFFBu kapı devre dışı.", source, 255, 0, 0, true)
					elseif (interiorStatus[3]) then
						...
						exports.global:sendLocalMeAction(source, "kapıya sertçe vurur ve kapının kiliti düşer.")
						...
						outputChatBox("#FF0000[!]#FFFFFFBu kapı kitli değil.", source, 0, 0, 0, true)
					end
				end
			--[[else
		...
		elseif (itemID==68) then
			exports.global:sendLocalMeAction(source, "looks at their lottery ticket.")
		...
		elseif itemID == 132 then
			...
			exports.global:sendLocalMeAction(source, "takes some prescription medicine.")
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
		elseif itemID == 134 then
			outputChatBox("#FF0000[!]#FFFFFF" .. exports.global:formatMoney(itemValue) .. "₺ paranız var..", source, 0, 0, 0, true)
		elseif itemID == 137 then
			if (getElementData(source, "snake")==true) then
				triggerEvent( "stopSnakeCam", source, source )
			else
				triggerEvent( "startSnakeCam", source, source )
			end
		elseif itemID == 355 then
			takeItemFromSlot(source, itemSlot)
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "bir et dürüm yer.")
			setElementHealth(source, getElementHealth(source)+25)
			triggerClientEvent(source,"mangal:durumYe", source)
		...
		elseif badges[itemID] then
			if ( getElementData( source, badges[itemID][1] ) ) then
				...
				if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 then
				exports.global:sendLocalMeAction( source, "removes a " .. badges[itemID][1] .. " from their head." )
				else
				exports.global:sendLocalMeAction(source, "removes a " .. badges[itemID][1] .. ".")
				end
			else
				for key, badge in pairs ( badges ) do
					if key ~= itemID then
						if ( getElementData ( source, badge[1] ) ) then
							...
							if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 then
							exports.global:sendLocalMeAction( source, "sağ elini omuz bölgesine götürür ve rozeti çıkartarak arka cebine koyar." )
							else
							exports.global:sendLocalMeAction( source, "sağ elini omuz bölgesine götürür ve rozeti çıkartarak arka cebine koyar " )
							end
						end
				...
				if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 then
				exports['anticheat-system']:changeProtectedElementDataEx( source, badges[itemID][1], true )
				exports.global:sendLocalMeAction( source, "wraps a " .. badges[itemID][1] .. " around their head." )
				else
				exports['anticheat-system']:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(itemValue), true )
				exports.global:sendLocalMeAction( source, "sağ elini arka cebine atar ve rozetini sol omuz bölgesine getirir ve takar." )
				end
			end
		...
		elseif itemID == 139 then
			outputChatBox("((Have a super+ Admin install this in your vehicle.))", source, 255, 0, 0)
		elseif itemID == 143 then
			outputChatBox("Place this device in a vehicle inventory to install it.", source, 0, 255, 0)
		elseif itemID == 145 then
			triggerClientEvent(source, "onFlashlightToggle", root)
		elseif (itemID==226) then
			if not (getElementData(source, "bayrak")) then
				bayrak = createObject(2914, 0, 0, 0, 0, 180, 0, true)
				local texture = getItemTexture(226, itemValue)
				if texture then
					for k,v in ipairs(texture) do
						exports["item-texture"]:addTexture(bayrak, v[2], v[1])
					end
				end
				...
				setElementData(source, "bayrak", bayrak)
			else				
				local bayrak = getElementData(source, "bayrak")
				exports.bone_attach:detachElementFromBone(bayrak)
				destroyElement(bayrak)
				bayrak = nil
				setElementData(source, "bayrak", false)
		...
		elseif (itemID>=448) and (itemID<=460) then
			exports["wearable-system"]:triggerWearableSystem(source, itemID, itemSlot)
		--[[elseif (itemID >= 290 and itemID <= 298) then
			...
			outputChatBox("Hata Kodu" .. ("%04d"):format(tonumber(itemID) or 9999) .. " - Eşya İşlemi Geçersiz.", source, 255, 0, 0)
		end
	end
end
...
function destroyItem(itemID)
	...
	outputChatBox("#ff0000[!]Üstündeki eşyayı sildin. #ffffff[" .. itemName .. "]", source, 0, 0, 0, true)
	exports.global:sendLocalText(source, " ** " .. getPlayerName(source):gsub("_", " ") .. " elini cebine/çantasına atar ve eşyayı alarak en yakınındaki çöp kutusuna fırlatır. ((" .. itemName .. "))", 255, 51, 102) 
end
...
function showItem(itemName)
	if isPedDead(source) or getElementData(source, "injuriedanimation") then return end
	exports.global:sendLocalMeAction(source, "shows everyone a " .. removeOOC(itemName) .. ".")
end
