...
local function output(from, to, itemID, itemValue)
	...
	if from ~= to and getElementType(from) == "player" and getElementType(to) == "player" then
		exports.global:sendLocalMeAction( from, "elini cebine atar ve üstündeki eşyayı " .. getPlayerName( to ):gsub("_", " ") .. "'a/e verir. ((" .. getItemName( itemID, itemValue ) .. "))" )
	-- player to item
	elseif from ~= to and getElementType(from) == "player" then
		...
		elseif itemID == 150 then --ATM card / MAXIME
			exports.global:sendLocalMeAction( from, "inserts an ATM card into the ATM machine's slot." )
		else
			exports.global:sendLocalMeAction( from, "üzerinde ki/çantasında ki eşyayı ((" .. getItemName( itemID, itemValue ) .. ")) eline alır ve aracın içine koyar. ((".. name .."))", 0,0, 0, true)
		end
	-- item to player
	elseif from ~= to and getElementType(to) == "player" then
		...
		if itemID == 134 then
			...
			exports.logs:dbLog(to, 6, from, "ARACTAN_OBJE_ALINDI")
			exports["vehicle-manager"]:addVehicleLogs(getElementData(from, "dbid"), "ARACTAN_OBJE_ALINDI", to)
		end
	end
end
...
local function moveToElement( element, slot, ammo, event )
	...
	local name = getElementModel( element ) == 2942 and "ATM Machine" or getElementModel( element ) == 2147 and "fridge" or getElementModel(source) == 3761 and "shelf" or ( getElementType( element ) == "vehicle" and "vehicle" or ( getElementType( element ) == "player" and "player" or "safe" ) )
				...
				if (item[1] == 134) then -- Money
					...
					if not success then
						if getElementModel(element) == 2942 then
							...
							outputDebugString( "ATM Error: " .. tostring(reason) )
						else
							outputDebugString( "Item Moving failed: " .. tostring( reason ))
						end
					else
						if getElementModel(element) == 2942 then
							exports["bank-system"]:playAtmInsert(element)
						end
...
local function moveFromElement( element, slot, ammo, index )
	...
	if item and item[3] == index then
		if not exports.global:isPlayerAdmin( source ) and getElementType( element ) == "vehicle" and ( item[1] == 61 or item[1] == 85  or item[1] == 117 or item[1] == 144) then
		...
		elseif item[1] == -100 then
			...
			if(getElementType(element) == "vehicle" and item[1] == 144) then --Yellow Roof Strobe
				removeVehicleSirens(element)
			end
			takeItemFromSlot( element, slot )
			output( element, source, item[1])
			if ammo < item[2] then
