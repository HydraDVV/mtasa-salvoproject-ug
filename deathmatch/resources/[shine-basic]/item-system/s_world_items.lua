...
function dropItemOnDead(itemID,itemValue, x, y, z, ammo, keepammo)
	if itemID then
		...
		if not ammo then
			...
			if itemID == 60 or itemID == 137 or itemID == 138 or itemID == 81 or itemID == 103 or itemID == 139 then
				return false
			else
				...
				if insert then
					...
					if itemID == 134 then
						outputChatBox("You left behind $" .. exports.global:formatMoney(itemValue) .. ".", source, 255, 194, 14)
					elseif (itemID == 80) and tostring(itemValue):find( ":" ) then
						outputChatBox("You left behind a " .. name .. ".", source, 255, 194, 14)
					else
						outputChatBox("You left behind a " .. getItemName( itemID, itemValue ) .. ".", source, 255, 194, 14)
					end
					...
					exports.logs:dbLog(source, 39, source, getPlayerName(source) .. " -> ~World(Dead) #" .. getElementID(obj) .. " - " .. getItemName( itemID ) .. " - " .. itemValue )
			...
			if tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
				return false
			else
				...
				outputChatBox("You left behind a " .. ( getWeaponNameFromID( itemID ) or "Body Armor" ) .. ".", source, 255, 194, 14)
				...
				if query then
					...
					exports.logs:dbLog(source, 39, source, getPlayerName(source).. " -> ~World(Dead) #" .. getElementID(obj) .. " - " .. getItemName( -itemID ) .. " - " .. ammo)
				end
			end
		end
...
function dropItem(itemID, x, y, z, ammo, keepammo)
	...
	if not ammo then
		...
		if itemID == 134 then
			...
			if hoursPlayed < 999999 then
				outputChatBox("Bug kullanım yasaklandı..", source, 255, 0, 0)
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.global:takeMoney(source, itemValue)
		...
		elseif itemID == 139 then
			...
			if insert then
				...
				elseif (itemID == 80) and tostring(itemValue):find( ":" ) then
				outputChatBox("#ff0000[!]#ffffffEşyayı yere attın. "..name.." . ",source,0,0,0,true)
				else
					outputChatBox("#ff0000[!]#ffffffEşyayı yere attın. ((" .. getItemName( itemID, itemValue ) .. "))", source, 0, 0, 0, true)
				end
				...
				elseif (itemID==80) and tostring(itemValue):find( ":" ) then
					exports.global:sendLocalMeAction(source, " sağ ve sol eli ile tutarak yere bırakır. #ffffff((" .. name .. "))",0,0,0, true)
				else
					exports.global:sendLocalMeAction(source, "sağ ve sol eli ile tutarak yere bırakır. ((" .. getItemName( itemID, itemValue ) .. "))", 0,0,0, true)
				end
			end
		end
	else
...
function pickupItem(object, leftammo)
	...
	if (getDistanceBetweenPoints3D(x, y, z, ox, oy, oz)<10) then
			...
			if tonumber(creatorAccountID) == tonumber(pickupAccountID) and (creatorPlayer ~= pickupPlayer) then
				--if (creatorPlayer == pickupPlayer) then
					...
					elseif itemID == 150 then
						exports.global:sendMessageToAdmins("AdmWrn: " .. pickupPlayer:gsub("_", " ") .. " tried to alt-alt their ATM card from " .. creatorPlayer:gsub("_", " ") .. ".")
					end
					exports.logs:dbLog(source, 5, source, "TRIED TO ALT-ALT ITEM ID: " .. itemID .. " | VALUE: " .. itemValue)
					return
				else
					if itemID == 134 then -- holy FUCK money duping bug.
		...
		--[[elseif (itemID==80) and itemValue:find( ":" ) then 
			...
			outputChatBox("#ffff00[!]Eşyayı yerden aldın.#ffffff((" .. getItemName( itemID, itemValue ) .. "))", source, 0, 0, 0, true)
			exports.global:sendLocalMeAction(source, "sağ ve sol elini eşyaya uzatarak eşyayı yerden alır. ((" .. getItemName( itemID, itemValue ) .. "))")
		end
