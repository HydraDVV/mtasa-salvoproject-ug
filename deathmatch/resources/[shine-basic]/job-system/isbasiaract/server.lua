mysql = exports.mysql
local temizlikYeri = createColSphere ( 2717.7412109375, -2399.384765625, 13.6328125, 3)
local meslekID = 30
	    ...
	    if not (getElementData(thePlayer, "job") == meslekID) then
	        if not (getElementData(thePlayer, "job") == 0) then
	            ...
	            if setElementData(thePlayer, "job", tonumber(meslekID)) then
	                ...
	                outputChatBox("#00CC00[!]#ffffffMeslek başarıyla alındı: #6699FFAraç Taşımacılık Mesleği", thePlayer, 255, 194, 14, true )
	            end
		    end
		else
		outputChatBox("#cc0000[!]#ffffffZaten Araç Taşımacılık Mesleğindesiniz.", thePlayer, 255, 0, 0, true )
	    end
	else	
	    outputChatBox("#cc0000[!]#ffffffMeslek bölgesinde değilsin.", thePlayer, 255, 0, 0, true )
	end
end
addCommandHandler("aractasimaisbasi", temizlik)