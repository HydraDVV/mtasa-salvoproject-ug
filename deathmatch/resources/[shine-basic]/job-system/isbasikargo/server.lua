mysql = exports.mysql
local temizlikYeri = createColSphere ( 1261.8642578125, -1842.8251953125, 13.554958343506, 3)
local meslekID = 28
	    ...
	    if not (getElementData(thePlayer, "job") == meslekID) then
	        if not (getElementData(thePlayer, "job") == 0) then
	            ...
	            if setElementData(thePlayer, "job", tonumber(meslekID)) then
	                ...
	                outputChatBox("#00CC00[!]#ffffffMeslek başarıyla alındı: #6699FFKargo Mesleği", thePlayer, 255, 194, 14, true )
	            end
		    end
		else
		outputChatBox("#cc0000[!]#ffffffZaten Kargo Mesleğindesiniz.", thePlayer, 255, 0, 0, true )
	    end
	else	
	    outputChatBox("#cc0000[!]#ffffffMeslek bölgesinde değilsin.", thePlayer, 255, 0, 0, true )
	end
end
addCommandHandler("kargoisbasi", temizlik)