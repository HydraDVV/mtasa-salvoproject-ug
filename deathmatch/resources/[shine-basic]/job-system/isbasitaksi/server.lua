mysql = exports.mysql
local temizlikYeri = createColSphere ( 1742.751953125, -1862.8515625, 13.575789451599, 3)
local meslekID = 29
	    ...
	    if not (getElementData(thePlayer, "job") == meslekID) then
	        if not (getElementData(thePlayer, "job") == 0) then
	            ...
	            if setElementData(thePlayer, "job", tonumber(meslekID)) then
	                ...
	                outputChatBox("#00CC00[!]#ffffffMeslek başarıyla alındı: #6699FFTaksi Mesleği", thePlayer, 255, 194, 14, true )
	            end
		    end
		else
		outputChatBox("#cc0000[!]#ffffffZaten Taksi Mesleğindesiniz.", thePlayer, 255, 0, 0, true )
	    end
	else	
	    outputChatBox("#cc0000[!]#ffffffMeslek bölgesinde değilsin.", thePlayer, 255, 0, 0, true )
	end
end
addCommandHandler("taksiisbasi", temizlik)