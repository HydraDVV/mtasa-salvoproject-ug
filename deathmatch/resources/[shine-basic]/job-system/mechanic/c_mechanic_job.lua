	...
	if factionType == 7 then -- Mechanic Faction / Adams]]
		if not vehicle then
			...
			if not (wMechanic) then
				-- Create the window
				wMechanic = guiCreateWindow(X, Y, Width, Height, "Tamir Seçenekleri", false )
				...
				if not noTyres[getVehicleType(vehicle)] then
					bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Lastik Tamiri - $10", true, wMechanic )
				...
				--if (playerDimension > 0) then
					...
					if faction == 4 or ( tonumber( getElementData(vehicle, "job") or 0 ) == 0 and ( getElementData(vehicle, "faction") == -1 or getElementData(vehicle, "faction") == faction ) ) then
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Araba Boyama - $100", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Far Boyama - $4,000", true, wMechanic )
...
function tyreWindow()
	...
	if not (wTyre) then
		-- Create the window
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Tekerlek Seçenekleri.", false )
...
function paintColor( colorIndex )
	...
	exports.colorblender:openPicker("paintjob", string.format("#%02X%02X%02X", r, g, b) , "Far rengini belirleyiniz:")
end
...
function paintWindow()
	...
	if not (wPaint) then
		wPaint = guiCreateWindow(windowX, windowY, windowWidth, windowHeight, "Araba Boyası", false )
		bPaintColor1 = guiCreateButton( 0.05, 0.1, 0.45, 0.2, "Boyama Seçenekleri 1", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor1,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 1 )
			end
		end, false)	 
		bPaintColor2 = guiCreateButton( 0.55, 0.1, 0.45, 0.2, "Boyama Seçenekleri 2", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor2,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 2 )
			end
		end, false)	
		bPaintColor3 = guiCreateButton( 0.05, 0.4, 0.45, 0.2, "Boyama Seçenekleri 3", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor3,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 3 )
			end
		end, false)	
		bPaintColor4 = guiCreateButton( 0.55, 0.4, 0.45, 0.2, "Boyama Seçenekleri 4", true, wPaint )
		...
		bPaintClose = guiCreateButton( 0.05, 0.85, 0.90, 0.1, "Vazgeç", true, wPaint )
...
function paintjobWindow()
	...
	if not (wPaintjob) then
		...
		-- Close
		bPaintjobClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Vazgeç", true, wPaintjob )
