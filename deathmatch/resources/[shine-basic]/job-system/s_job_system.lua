...
function quitJob(source)
	...
	if logged == 1 then
		...
		if job == 0 then
			outputChatBox("[!]#ffffffHerhangi bir meslekte bulunmuyorsunuz.", source, 255, 0, 0, true)
		else
			local charID = getElementData(source, "dbid")
...
addCommandHandler("meslektenayril", quitJob, false, false)
...
function startEnterVehJob(thePlayer, seat, jacked) 
	...
	if vjob>0 and job~=vjob and seat == 0 and not (getElementData(thePlayer, "duty_admin") == 1) and not (getElementData(thePlayer, "duty_supporter") == 1) then
		...
		elseif (vjob==2) then
			--outputChatBox("[!] #FFFFFFBu aracı kullanabilmek için, Taksi Şöförlüğü mesleğinde olmanız gerekmektedir.", thePlayer, 255, 0, 0, true)
		elseif (vjob==3) then
			--outputChatBox("[!] #FFFFFFBu aracı kullanabilmek için, Otobüs Şöförlüğü mesleğinde olmanız gerekmektedir.", thePlayer, 255, 0, 0, true)
		end
...
function meslekAntiNJ(thePlayer, seat, jacked) 
	local vjob = tonumber(getElementData(source, "job")) or 0
	local job = getElementData(thePlayer, "job")
	local seat = getPedOccupiedVehicleSeat(thePlayer)
	local occupant = getVehicleOccupant(source, 0)

	if vjob>0 and not (getElementData(thePlayer, "duty_admin") == 1) then
		if not jacked then
			return
		else
			outputChatBox("Dikkat! NJ yapmaya çalıştınız! Bunu yapmaya devam ederseniz cezalandırılırsınız!", thePlayer, 255,0,0,true)
			exports.global:sendMessageToAdmins("AdmWrn: '" .. getPlayerName(thePlayer):gsub("_", " ") .. "' isimli oyuncu bir meslek aracına NJ yapmaya çalıştı.")
			cancelEvent()
		end
	end
end
addEventHandler("onVehicleStartEnter", getRootElement(), meslekAntiNJ)
--[[ -- Removed by MAXIME  
