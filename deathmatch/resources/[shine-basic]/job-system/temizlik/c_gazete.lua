				...
				for _, marker in ipairs(GazeteCreatedMarkers) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							outputChatBox("[!] #FFFFFFYeni rotanız belirleniyor, lütfen bekleyiniz. Eğer devam etmek istemiyorsanız, /Gazetebitir yazınız.", 0, 0, 255, true)
							setTimer(
...
addCommandHandler("gazetebitir", GazeteBitir)
...
function GazeteAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 574 and vehicleJob == 22 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 22 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için Yol Gazeteçisi olmanız gerekmektedir.", 255, 0, 0, true)
		end
	end
end
