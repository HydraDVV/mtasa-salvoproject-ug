...
function onLicenseServer()
	...
	if (gender == 0) then
		exports.global:sendLocalText(source, "Emily Smith says: Hello Sir, do you want to apply for a license?", nil, nil, nil, 10)
	else
		exports.global:sendLocalText(source, "Emily Smith says: Hello Ma'am, do you want to apply for a license?", nil, nil, nil, 10)
	end
end
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>5) then -- Are they standing next to each other?
						...
						outputChatBox("        Gun License: " .. guns, targetPlayer, 255, 194, 14)
	...
	if getElementData(source, "owner") == -2 and getElementData(source, "faction") == -1 and getElementModel(source) == 410 then
		if getElementData(player,"license.car") == 3 then
			if getElementData(player, "license.car.cangetin") then
				...
				outputChatBox("(( This DMV Car is for the Driving Test only, please see the NPC inside first. ))", player, 255, 0, 0)
				cancelEvent()
			end
		elseif seat > 0 then
