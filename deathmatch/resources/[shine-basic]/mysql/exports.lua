...
addEventHandler("onResourceStart", getResourceRootElement(),
	function ()
		serverDatabase = dbConnect("mysql", "dbname=shine;host=localhost;charset=utf8", "root", "123456789", "multi_statements=1")
		
		if not serverDatabase then
			outputDebugString("Offline!", 1)
			cancelEvent()
		else
			--dbExec(serverDatabase, "SET NAMES utf8")
			outputDebugString("Online!")
		end
	end
)
...
function connectToDatabase(res)
	MySQLConnection = mysql_connect("localhost", "root", "123456789", "shine", "3306")
