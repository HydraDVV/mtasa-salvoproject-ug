...
function payWage(player, pay, faction, tax)
	...
	cP = 0.005
	local moneyonhand = getElementData(player, "money")
	...
	local interest = math.ceil(tonumber(cP) * bankmoney)
	if (interest >  4999) then
		interest = 5000
	end
	
	if interest ~= 0 then
		dbExec(exports.mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (-57, " .. getElementData(player, "dbid") .. ", " .. interest .. ", 'BANKINTEREST', 6)" )
	end
	...
	if not faction then
		...
		elseif pay > 0 then
			...
			dbExec(exports.mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (-3, " .. getElementData(player, "dbid") .. ", " .. pay .. ", 'STATEBENEFITS', 6)" )
		else
			pay = 0
		end
	else
		if pay > 0 then
			...
			dbExec(exports.mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. teamid .. ", " .. getElementData(player, "dbid") .. ", " .. pay .. ", 'WAGE', 6)" )
		else
			pay = 0
		end
	end
	...
	if tax > 0 then
		...
		dbExec(exports.mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. getElementData(player, "dbid") .. ", -3, " .. tax .. ", 'INCOMETAX', 6)" )
	end
	...
	if vtax > 0 then
		...
		dbExec(exports.mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. getElementData(player, "dbid") .. ", -3, " .. vtax .. ", 'VEHICLETAX', 6)" )
	end
	...
	if ptax > 0 then
		...
		dbExec(exports.mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. getElementData(player, "dbid") .. ", -3, " .. ptax .. ", 'PROPERTYTAX', 6)" )
	end
	...
	if (rent > 0) then
		if (rent > bankmoney)   then
			...
			dbExec(exports.mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. getElementData(player, "dbid") .. ", 0, " .. rent .. ", 'HOUSERENT', 6)" )
		end
	end
...
function payAllWages(timer)
	...
	govAmount = exports.global:getMoney(getTeamFromName("Government of İstanbul"))
...
function resumeThreads()
	...
	if not inFor then	
		...
		exports.global:setMoney(getTeamFromName("İstanbul Büyükşehir Belediyesi"), govAmount)
...
function doPayDayPlayer(value, hourly)
	...
	local dbid = getElementData( value, "dbid" )
		...
		setElementData(value, "timeinserver", math.max(0, timeinserver-60))
		local hoursplayed = getElementData(value, "hoursplayed") or 0
		setElementData(value, "hoursplayed", hoursplayed+1)
		dbExec(exports.mysql:getConnection(), "UPDATE characters SET hoursplayed = hoursplayed + 1, bankmoney = " .. getElementData( value, "bankmoney" ) .. sqlupdate .. " WHERE id = " .. dbid )
		totTxt = ""

	   for i = 1,2 do
	   local asa = string.char(math.random(48,57))
		local asb = asa..string.char(math.random(65,90))
	       totTxt = totTxt..asb
	   end
	--   setElementData(value, "saatlikOnay", true)
	 --  setElementData(value, "saatlikKod", totTxt)
	   triggerClientEvent(value, "paraGelmeSesiCalistir", value)
	--   outputChatBox("[!] #D6D6D6Saatlik bonus ve maaşınızı 2 dakika içinde [/onayla "..totTxt.."] yazarak onaylayıp alabilirsiniz.", value, 0, 55, 255, true)
	   outputChatBox("[!] #FFFFFFTebrikler Salvo saatlik bonus'dan 5.000₺ kazandınız!",value,43,255,96,true)
	   exports.global:giveMoney(value, 5000)
	   end
...
function loadWelfare( )
	...
	if result then
		if not result.value then
			dbExec(exports.mysql:getConnection(), "INSERT INTO settings (name, value) VALUES ('welfare', " .. unemployedPay .. ")" )
		else
