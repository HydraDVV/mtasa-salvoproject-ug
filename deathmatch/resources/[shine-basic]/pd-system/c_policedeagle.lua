...
function switchMode()
	if (getPedWeapon(localPlayer)==24) and (getPedTotalAmmo(localPlayer)>0) then -- has an un-empty deagle
		...
		if mode == 0 then -- tazer mode
			...
			outputChatBox( "#FF0000[!]#FFFFFFSilah şuan seri atış(lethal) durumundadır.", 0, 0, 0, true )
		elseif mode == 1 and getElementData(team, "type") == 2 then -- lethal mode
			outputChatBox( "#FF0000[!]#FFFFFFSilahınız şuan Araç Hız(Radar) takip modundadır.", 0, 0, 0, true )
			triggerServerEvent("deaglemode", localPlayer, 2)
		elseif mode == 2 or mode == 1 and getElementData(team, "type") == 2 then -- radar gun mode
			outputChatBox( "#FF0000[!]#FFFFFFSilahınız şuan elektro şok(tazer) modundadır..", 0, 0, 0, true )
		...
		if (getElementType(target)=="vehicle") then
			if (getPedWeapon(localPlayer)==24) then
				...
				if (mode==2) then
					...
					outputChatBox(getVehicleName(target) .. " clocked in at " .. math.floor(actualspeed) .. " km/h.", 255, 194, 14)
				end
			end
		end
	end
