...
-- /pdcodes
function showTheShitThing(thePlayer)
   local theTeam = getPlayerTeam(thePlayer)
   local factionType = getElementData(theTeam, "type")
      
   if tonumber(getElementData(thePlayer, "faction")) == 1 then
      triggerClientEvent ( "showPoliceWindow", thePlayer)
   end
end
addCommandHandler("pdcodes", showTheShitThing)
			...
			if not (targetPlayerNick) then
				...
				if targetPlayer then
					...
					if (distance<=10) then
						...
						outputChatBox(targetPlayerName .. "'ın parkmak izi: " .. tostring(fingerprint) .. ".", thePlayer, 255, 194, 14)
					else
...
addCommandHandler("parmakizial", fingerprintPlayer, false, false)
	...
	if (logged==1) then
		...
		if (factionType==2) then
			if not (targetPlayerNick) or not (amount) or not (...) then
				outputChatBox("SYNTAX: /" .. commandName .. " [Oyuncunun Adı] [Ceza Parası] [Sebebi]", thePlayer, 255, 194, 14)
			else
				local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					if (distance<=10) then
						...
						outputChatBox("Ceza kesildi. " .. targetPlayerName .. " " .. exports.global:formatMoney(amount) .. ". Açıklama: " .. reason .. ".", thePlayer)
						outputChatBox("Kesilen miktar " .. exports.global:formatMoney(amount) .. " Cezayı kesen memur " .. getPlayerName(thePlayer) .. ". Sebeb: " .. reason .. ".", targetPlayer)
						if takeFromBank > 0 then
							outputChatBox("Size kesilen ceza miktarı $" .. exports.global:formatMoney(takeFromBank) .. " yeterli miktar bulunmadığı için banka hesabınızdan kesilmiştir..", targetPlayer)
