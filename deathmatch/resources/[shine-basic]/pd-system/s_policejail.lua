	...
	if (logged==1) then
		...
		if (factionType==2) and playerCol then
			if not (targetPlayerNick) or not (fine) or not (jailtime) or not (...) or (jailtime<1) or (jailtime>1441) then
				outputChatBox("Kommut: /hapseat [Oyuncunun Adı/ ID] [Para Cezası] [(Süre) 1->1440)] [Sebep]", thePlayer, 255, 194, 14)
			else
				...
				if targetPlayer then
					...
					if not targetCol then
						outputChatBox("#FF0000[!]#FFFFFFOyuncu hapise atılacak alanda olması gerekmektedir..", thePlayer, 0, 0, 0, true)
					elseif targetCol ~= playerCol then
						outputChatBox("#FF0000[!]#FFFFFFOyuncu başka bir hücrenin önünde duruyor.", thePlayer, 0, 0, 0, true)
					else
						...
						if (jailTimer) then
							outputChatBox("#FF0000[!]#FFFFFFBu oyuncu zaten hapis cezası devam ediyor.", thePlayer, 0, 0, 0, true)
						else
							...
							outputChatBox("Seni hapise atan polis" .. username .. " || " .. jailtime .. " dakika(s).", targetPlayer, 0, 102, 255)
							outputChatBox("Sebep: " .. reason .. ".", targetPlayer, 0, 102, 255)
							if (finebank == true) then
								outputChatBox("#FF0000[!]#FFFFFFPara cezasının geri kalan kısmı banka hesabınızdan alınmıştır..", targetPlayer, 0, 0, 0, true)
							end
							
							for key, value in ipairs(teamPlayers) do
								outputChatBox(factionRankTitle .. " " .. username .. " hapise attı. " .. targetPlayerNick .. "" .. jailtime .. " dakika.", value, 0, 102, 255)
								outputChatBox("Sebep: " .. reason .. ".", value, 0, 102, 255)
							end
...
addCommandHandler("hapseat", arrestPlayer)
		...
		if ( timeServed ) then
			...
			if (timeLeft<=0) then
				...
				if (getElementData(jailedPlayer, "pd.jailstation") <= 4) then
					...
					setElementDimension(jailedPlayer, 3)
...
function showJailtime(thePlayer, commandName)
	...
	if ajailtime then
		outputChatBox("#FF0000[!]#FFFFFFSenin hapishaneden çıkmana  " .. ajailtime .. " dakika var.", thePlayer, 0, 0, 0, true)
	else
		outputChatBox("#FF0000[!]#FFFFFFŞuanda göz altındasın.", thePlayer, 0, 0, 0, true)
	end
	
	local isJailed = getElementData(thePlayer, "pd.jailtimer")
	if isJailed then
		local jailtime = getElementData(thePlayer, "pd.jailtime")
		outputChatBox("#FF0000[!]#FFFFFFGöz altının bitme süresi :  " .. jailtime .. "", thePlayer, 0, 0, 0, true)
	else
		outputChatBox("Sen tutuklu değilsin.", thePlayer, 255, 0, 0)
	end
end
	...
	if (logged==1) then
		...
		if factionType == 2 and isInArrestColshape(thePlayer) or exports.global:isPlayerAdmin(thePlayer) and getElementData(thePlayer, "adminduty") == 1 then
			if not (targetPlayerNick) then
				outputChatBox("SYNTAX: /hapistencikar [Oyuncu Adı / ID]", thePlayer, 255, 194, 14)
			else
				local targetPlayer, targetPlayerNick = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					if (jailTimer) then
						...
						outputChatBox("#FF0000[!]#FFFFFFBu oyuncu hapis cezasına çarptırılmıyor.", thePlayer, 0, 0, 0, true)
					end
				end
			end
		end
...
addCommandHandler("hapistencikar", jailRelease)