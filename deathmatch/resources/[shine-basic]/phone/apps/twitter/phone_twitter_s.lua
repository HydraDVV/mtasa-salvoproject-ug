...
local sortId = 0
local advertisementMessages = { "Anadolu", "polis","egm","sefa","yusuf","yusufsefa","anka","pars","diego","yarak", "lspd","fuck", "gel", "yarram", "yarrak", "oscuri", "burockobuma", "fire", "rp", "siker",  "samp", "aga", "RydeR", "ryder", "samp", "arıyorum", "aranır", "istiyom", "istiyorum", "SA-MP", "roleplay", "ananı", "sikeyim", "sikerim", "orospu", "evladı", "Kye", "arena", "Arina", "rina", "vendetta", "vandetta", "shodown", "Vedic", "vedic","ventro","Ventro", "server", "sincityrp", "ls-rp", "sincity", "tri0n3", "mta", "mta-sa", "query", "Query", "inception", "p2win", "pay to win" }
local spamTimers = {}
    ...
    function(player, name, location, text)
        ...
        for k,v in ipairs(advertisementMessages) do
            ...
            if (found) or (found2) or (string.lower(text)==tostring(v)) then
                ...
                outputChatBox("[!]#ffffff Tweet yazısında yasaklı kelimeler bulundu.", player, 255, 0, 0, true)
                return
            end
        end
        if twitk[player] then
        outputChatBox('[!]#ffffff Biraz yavaşlamaya ne dersin',player,255,0,0,true)
        return end
     ...
     --   if isTimer(spamTimers[getElementData(player, "dbid")]) then return end
       ...
       -- if getElementData(player, "vipver") >= 4 then
        --    triggerClientEvent(root, "addNotify", root, "twitter", name, text)
         --  exports["orp_infobox"]:addBox(root,"error", text)
        --end
        ...
        sync()
                exports["titan_infobox"]:addBox(root,"twitter",""  .. name .. ": " .. text .. "")    
        twitk[player] = true
        setTimer(function()
        twitk[player] = nil
        end, 100000, 1)
        --outputChatBox('[!]#FFFFFF Yeni bir tweet\'in var',root,0,0,255,true)
    end
)
