	...
	if aracID and miktar then
		...
		if arac then
			...
			if miktar > vergi then
				exports["ed_infobox"]:addBox(source, "warning", "Ödemeye çalıştığınız miktar, aracın vergi borcundan fazla.")
				return false
			end
			local playerMoney = getElementData(source, "money")
			if not exports.global:hasMoney(source, miktar) then
			exports["ed_infobox"]:addBox(source, "error", "Yeterli paranız yok.")
				return false			
			end
			...
			exports["ed_infobox"]:addBox(source, "success", "Başarıyla $" .. exports.global:formatMoney(miktar) .. " karşılığında araç verginizi ödediniz. Kalan Vergi: $" .. exports.global:formatMoney(kalanvergi).."")
