...
function createSetupGUIs()
	...
	addEventHandler("onClientGUIClick", setupBtn, function(state)
		if state == "left" then
			...
			if text:len() == 4 then
				outputChatBox("#575757Salvo:#ffffff Şifreniz kabul edildi, lütfen bekleyin.", 0, 255, 0, true)
	...
	addEventHandler("onClientGUIClick", setupBtn, function(state)
		if state == "left" then
			...
			if (tonumber(text) == tonumber(phonePassword)) then
				...
				outputChatBox("#575757Salvo:#ffffff Hatalı şifre girdiniz, tekrar deneyin.", 255, 0, 0, true)
			end
		end
	end)
end
...
function drawnPhone()
	...
	if phoneSetupState == 2 then
		...
		if appstoreActive then
			...
			--@draw AppStore
			c_apps = { ['twitter'] = false, ['safari'] = false, ['spotify'] = false, ['flappy-bird'] = false }
		...
		--	dxDrawImage(x,y,w,h,"components/images/iphone_front_twitter.png",0,0,0,tocolor(255,255,255))
			...
			dxDrawText(getNavbarTime(), x, y+40, x+w, y, tocolor(255,255,255),1,fonts[5],"center","top")
	...
	if drawApps then
		...
		c_apps = {"twitter", "safari", "spotify", "flappy-bird"}
		-- First check it:
		...
		for index, data in pairs(apps) do
			...
			if fileExists("components/images/"..app..".png") then
				...
				dxDrawImage(x+23+appsHeight,y+70,48,48,"components/images/"..app..".png")
				if isInSlot(x+23+appsHeight,y+70,48,48) then
					if getKeyState("mouse1") and lastClick+200 <= getTickCount() then
						lastClick = getTickCount()
						if app == "history" then
							toggleOffEverything()
...
function hidePhoneGUI()
	...
	closeConfirmBox()
	killSettingSounds()
	--exports.ed_ed_OwlGamingLogs:closeInfoBox()
end
...
function onGuiClick(button)
	...
	elseif button == "left" and phoneSetupState == 2 then
		...
		elseif source == bKonumAt then
			...
			--	outputChatBox("#575757Salvo:#ffffff Konum başarıyla atıldı!",0,255,0,true)
...
addEventHandler("konum:markerAc",root,function(p1,p2,x,y,z)
	...
	blip[p2] = createBlip(x,y,z,41)--waypoint blip
 	addEventHandler("onClientMarkerHit",marker[p2],function(hitElement)
		...
		outputChatBox("#575757Salvo:#ffffff Konuma ulaştınız!",0,255,0,true)
	end)
end)
...
addCommandHandler('bildirim',
	function(cmd,state)
		if (not state) then
			outputChatBox("#575757Salvo:#ffffff /bildirim [aç/kapat]",0,0,255,true)
			return
		end
		if (state == "aç") then
			outputChatBox("#575757Salvo:#ffffff Bildirimler başarıyla açıldı.", 0, 255, 0, true)
			localPlayer:setData('notifications->disabled', false)
		elseif (state == "kapat") then
			outputChatBox("#575757Salvo:#ffffff Bildirimler başarıyla kapatıldı.", 0, 255, 0, true)
