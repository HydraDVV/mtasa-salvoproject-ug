...
addEventHandler("fetchApps", root,
	function(phone)
		local apps =  { ['twitter'] = true, ['safari'] = true, ['spotify'] = true, ['emails'] = true, ['flappy-bird'] = true }
...
addEventHandler("completePhoneSetup", root,
	function(player, phoneNumber, phonePassword)
		...
		if exec then
			outputChatBox("#575757Salvo:#ffffff Telefon kurulumu tamamlandı, şifreniz: "..phonePassword,player,0,255,0,true)
			...
			function(res, rows, err, player, cardnumber, cardpassword)
				if rows > 0 then
					if res["card_pin"] == cardpassword then
						...
						outputChatBox("#575757Salvo:#ffffff Yanlış kart şifresi girdiniz. ( Eğer ilk defa aldıysanız şifre 0000'dır. )",player,255,0,0,true)
					end
				else
					outputChatBox("#575757Salvo:#ffffff Kart numarası bulunamadı.",player,255,0,0,true)
				end
			end
		)
	end
...
function applyPhone(string, popOutOnPhoneCall)
	...
	if string == "phone_in" then
		...
		if getElementData(source, "cellphoneGUIStateSynced") ~= 1 then
			exports['anticheat-system']:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", 1 , true)
		end
		exports['anticheat-system']:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", 1 , true)
	...
	elseif string == "phone_out" then
		...
		exports['anticheat-system']:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", nil , true)
		if isElement(phoneO[source]) then
...
function setEDX(thePlayer, index, newvalue, sync, nosyncatall)
	return exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, index, newvalue, sync, nosyncatall)
end
...
function cleanUpOnePlayer(player)
	...
	exports['anticheat-system']:changeProtectedElementDataEx(player, "cellphoneGUIStateSynced", nil, true)
end
	...
	for k,v in ipairs(getElementsByType("player")) do
		if getElementData(v,"loggedin") == 1 then
			if exports["global"]:hasItem(v,2,number) then
				found = found + 1
				if found == 1 then
					outputChatBox("#575757Salvo:#ffffff "..getPlayerName(player):gsub("_", " ").." adlı kullanıcı sana konum yolladı!",v,0,255,0,true)
					outputChatBox("#575757Salvo:#ffffff SMS ile konum gönderildi, gitmek istediğin alan blip ile işaretlenecek.",v,0,255,0,true)
					return v
				else
					outputChatBox("#575757Salvo:#ffffff Konum atmak istediğiniz oyuncu oyunda olmadığı için yalnızca SMS gönderildi!",player,255,0,0,true)
					return false
				end
			end
		end
			...
			for key, value in ipairs(exports.pool:getPoolElementsByType("player")) do
				if getElementData(value, "loggedin")==1 then
					...
					if hasTogAd and tonumber(togAdState) == 1 then
						...
						if exports.integration:isPlayerAdmin(value) then
...
function SAN_listAdverts(thePlayer)
	if exports.global:isPlayerDeveloper(thePlayer)  then
...
function SAN_freezeAdvert(thePlayer, commandHandler, advertID)
	if exports.ed_integration:isPlayerTrialAdmin(thePlayer) or exports.ed_integration:isPlayerSupporter(thePlayer) then
...
function addPhone(thePlayer, commandName)
	if (exports.ed_integration:isPlayerTrialAdmin(thePlayer)) then
