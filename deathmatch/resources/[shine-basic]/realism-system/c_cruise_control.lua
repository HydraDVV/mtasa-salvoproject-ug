local limitSpeed = { }
table.insert(limitSpeed, 510, 85) -- Mountain bike
table.insert(limitSpeed, 509, 80) -- Bike
table.insert(limitSpeed, 481, 80) -- BMX
...
function applyCruiseControl()
    ...
    if (theVehicle) then
        if (getVehicleOccupant(theVehicle) == getLocalPlayer()) then
            if (ccEnabled) then
                outputChatBox("Hýz sabitleyici devredýsý", 255,0,0)
                deactivateCruiseControl()
            else
                targetSpeed = getElementSpeed(theVehicle)
                if targetSpeed > 4 then
                    if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train") then
                        outputChatBox("#FF0000[!]#FFFFFFHız Sabitleyici Aktif", 0,0,0, true)
                        outputChatBox("#FF0000[!]#FFFFFFHızınızı yükseltmek veya düşürmek için - ve + 'ya basınız", 0,0,0, true)
                        activateCruiseControl()
                    end
                else
                    outputChatBox("#FF0000[!]#FFFFFFHız sabitleyici hızınızı sürekli sabit tutmanızı sağlar.", 0,0,0, true)
                end
            end
        end
    end
