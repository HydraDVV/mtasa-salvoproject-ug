...
function delayedKeyPress( key, state )
	...
	setTimer( setControlState, 100, 1, key, ( state == "down" ))
end
		...
		if getElementData(source, name) then
			...
			elseif getElementData(source, name) == 0 then
				...
				if alcohollevel < 1 then
					...
					drunktimer = setTimer(function() local level = getElementData(getLocalPlayer(), "alcohollevel") setElementData(getLocalPlayer(), "alcohollevel", 0, false) end, 600000, 0)
				else
					-- random effect
