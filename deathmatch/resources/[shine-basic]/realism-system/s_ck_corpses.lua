...
function addCharacterKillBody( x, y, z, rotation, skin, id, name, interior, dimension, age, race, weight, height, desc, cod, gender )
	...
	exports['anticheat-system']:changeProtectedElementDataEx(ped, "ckid", id, false)
	killPed(ped)
...
addEventHandler("ck:info", getRootElement(),
	function()
		if CKs[source] then
			triggerClientEvent(client, "ck:show", client, CKs[source].text, exports.global:isPlayerFullAdmin(client) and CKs[source].name)
		end
	end
)
