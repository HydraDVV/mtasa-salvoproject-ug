...
local skins = { { 211, 217 }, { 179 }, false, { 178 }, { 82 }, { 80, 81 }, { 28, 29 }, { 169 }, { 171, 172 }, { 142 }, { 171 }, { 171, 172 }, {71}, { 50 }, {308} }
				...
				if (pedType) then
					if (pedType=="shop") then
						...
						if (tonumber(id)==dbid) then
							...
							dbExec(mysql:getConnection(),"DELETE FROM shops WHERE id='" .. mysql:escape_string(dbid) .. "' LIMIT 1")
...
function loadAllGeneralshops(res)
    ...
    mysql:getConnection(), "SELECT id, x, y, z, dimension, interior, shoptype, rotation, skin FROM shops")
end
...
addEventHandler( "shop:buy", resourceRoot, function( index )
	...
	if isTimer(spamTimers[source]) then
		local remainingSeconds = math.ceil(getTimerDetails(spamTimers[source])/1000)
		outputChatBox("[!] #FFFFFFBu işlemi yapabilmek için "..remainingSeconds.." saniye bekleyiniz.", client, 255, 0, 0, true)
			return
		end
	spamTimers[source] = setTimer(function() end, 3 * 1000, 1) 	
	...
	if dimension > 0 then -- is even in any interior
		...
		if ownerID == 0 then
			...
			suppliesToTake = item.supplies or math.ceil( 3.5 * tonumber(exports['item-system']:getItemWeight( itemID, itemValue )) )
	...
	if exports.global:giveItem( client, itemID, itemValue ) then
		...
		--outputChatBox( "#FF0000[!]#FFFFFFMarketten bir ürün ((" .. item.name .. "))" .. exports.global:formatMoney( price ) .. " Kurus/TL karşılığında satın aldınız .", client, 0, 0, 0, true )
		exports["titan_infobox"]:addBox(client,"success",item.name .. " isimli ürünü $" .. exports.global:formatMoney( price ) .. " karşılığında satın aldınız") 	 		
		--take the outstanding supplies
		
		derpface = item.supplies or math.ceil( 3.5 * tonumber(exports['item-system']:getItemWeight( itemID, itemValue )) )
			
		if dimension > 0 then
			dbExec(mysql:getConnection(),"UPDATE interiors SET supplies = supplies - " .. mysql:escape_string(derpface) .. " WHERE id = " .. mysql:escape_string(dimension))
		elseif dimension == 0 or (tonumber(result["type"]) == 2 or nil) then
			updateGlobalSupplies(-suppliesToTake)
		end
		...
		if itemID == 2 then
			dbExec(mysql:getConnection(),"INSERT INTO `phones` (`phonenumber`, `boughtby`) VALUES ('"..tostring(itemValue).."', '"..mysql:escape_string(tostring(getElementData(client, "account:character:id") or 0)).."')")
			outputChatBox("Yeni telefon numaranız 0" .. itemValue .. ".", client, 255, 194, 14 )
		elseif itemID == 16 and item.fitting then -- it's a skin, so set it.
			setElementModel( client, itemValue )
			dbExec(mysql:getConnection(),"UPDATE characters SET skin = " .. mysql:escape_string( itemValue ) .. " WHERE id = " .. mysql:escape_string(getElementData( client, "dbid" )) )
			if exports['anticheat-system']:changeProtectedElementDataEx( client, "casualskin", itemValue, false ) then
				dbExec(mysql:getConnection(),"UPDATE characters SET casualskin = " .. mysql:escape_string( itemValue ) .. " WHERE id = " .. mysql:escape_string(getElementData(client, "dbid")) )
			end
		...
		if price > 0 and dimension > 0 then
			...
			if ownerID > 0 then -- someone even owns it
				if ownerPlayer then
					...
					dbExec(mysql:getConnection(), "UPDATE characters SET bankmoney=bankmoney + " .. mysql:escape_string(price) .. " WHERE id = " .. mysql:escape_string(ownerID) .. " LIMIT 1")
				end
			end
		end
	else
		exports["titan_infobox"]:addBox(client,"error","Envanterinde boş yerin yok! Satın alamadığın ürün:" .. item.name .. ".") 	 
		--outputChatBox( "#FF0000[!]#FFFFFFÜrünü almak için yeterli miktarda boş yeriniz yok. #FF0000Ürün : " .. item.name .. ".", client, 0, 0, 0, true )
	end
end )
...
function updateGlobalSupplies(value)
	if value > 0 then	
		dbExec(mysql:getConnection(),"UPDATE settings SET value = value + '" .. mysql:escape_string(tostring(value)) .. "' WHERE name='globalsupplies'")
	elseif value < 0 then
		dbExec(mysql:getConnection(),"UPDATE settings SET value = value - '" .. mysql:escape_string(tostring(value)) .. "' WHERE name='globalsupplies'")
	end
end
...
function handleSupplies(element, slot, event, worldItem)
	...
	local result = dbExec(mysql:getConnection(),"UPDATE interiors SET supplies= supplies + " .. mysql:escape_string(amount) .. " where id='" .. mysql:escape_string(dbid) .. "'")
	if result then
