				...
				for k, arrayPlayer in ipairs(exports.global:getAdmins()) do
					...
					if (logged) then
						if exports.global:isPlayerLeadAdmin(arrayPlayer) then
							outputChatBox( "LeadAdmWarn: " .. getPlayerName(thePlayer) .. " tüm arabalar respawnlıyor.", arrayPlayer, 255, 194, 14)
						end
					end
				end
				
				outputChatBox("*[!]Tüm arabaların yenilenmesine "..timeToRespawn.." saniye! ***", getRootElement(), 255, 194, 14)
				outputChatBox("Durdurmak için /respawnstop!", thePlayer)
...
function respawnVehiclesStop(thePlayer, commandName)
	if exports.global:isPlayerAdmin(thePlayer) and isTimer(respawnTimer) then
		...
		if commandName then
			...
			outputChatBox( "[!] " .. name .. " Yenilenmeyi durdurdu. ***", getRootElement(), 255, 194, 14)
		end
	end
end
...
function respawnAllCivVehicles(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		outputChatBox(" =-=-=-=-=-=- Tüm sivil arabalar yenilendi. =-=-=-=-=-=-=", getRootElement(), 255, 194, 14)
		outputChatBox("Yenilen Araç Sayısı :  " .. counter .. " araba.", thePlayer)
	end
end
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (veh) then
						...
						outputChatBox("Arabanız yenilendi " .. targetPlayerName .. "", thePlayer)
						outputChatBox("Arabanızı yenileyen Admin ||  " .. username .. ".", targetPlayer)
					else
						outputChatBox("Şuan yenileyecek araba yok.", thePlayer, 255, 0, 0)
					end
				end
			end
		end
...
function fixAllVehicles(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		executeCommandHandler("ann", thePlayer, "Tüm arabalarınız yenileniyor. Yenileyen Admin || " .. username .. ".")
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (veh) then
						...
						outputChatBox("Benzininiz fullendi. " .. targetPlayerName .. "", thePlayer)
						outputChatBox("Benzininizi yenileyen Admin ||  " .. username .. ".", targetPlayer)
						exports.logs:dbLog(thePlayer, 6, { targetPlayer, veh  }, "FUELVEH")
						
						addVehicleLogs(getElementData(veh,"dbid"), commandName, thePlayer)
						
					else
						outputChatBox("Benzininizi yenileyecek admin yok.", thePlayer, 255, 0, 0)
					end
				end
			end
		end
...
function fuelAllVehicles(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		executeCommandHandler("ann", thePlayer, "Tüm arabaların benzinleri dolduruldu. Yenileyen Admin ||  " .. username .. ".")
				...
				--if exports['vehicle-system']:hasVehiclePlates(theVehicle) then
					...
					if (exports['vehicle-plate-system']:checkPlate(plateText)) then
						...
						--[[local x, y, z = getElementPosition(theVehicle)
							...
							local dim = getElementDimension(theVehicle)
							exports['vehicle-system']:reloadVehicle(tonumber(vehicleID))
							local tnvehicle = exports.pool:getElement("vehicle", vehicleID)
							setElementPosition(tnvehicle, x, y, z)
							setElementInterior(tnvehicle, int)
							setElementDimension(tnvehicle, dim)
							old before setVehiclePlateText was added back in and fixed]]
							setVehiclePlateText(theVehicle, tostring(plateText))
							outputChatBox("Done.", thePlayer)
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 20 then
					...
					if theVehicle then
						...
						if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") or exports.global:isPlayerSuperAdmin(thePlayer) or (exports.donators:hasPlayerPerk(thePlayer, 16) and exports.global:isPlayerAdmin(thePlayer)) then
...
addCommandHandler("fsellc", sellVehicle)