				...
				if engine == 0 then
					...
					if (vehKey) or (owner < 0) and (faction == -1) or (playerFaction == faction) and (faction ~= -1) or ((getElementData(source, "duty_admin") or 0) == 1) then
						...
						if broke == 1 then
							...
							triggerEvent('sendAme', source, "anahtarı aracın kontağına takar ve saat yönünde çevirir.")
							setTimer(
								function()
									triggerEvent('sendAdo', source, "Aracın motoru bozuktur, araç çalışmaz.")
									outputChatBox("Aracın motoru bozuk.", source)
				...
				--if (owner ~= -1) then
					if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
						...
						if (locked) then
							...
							--exports.global:sendLocalMeAction(source, "Sağ elini, orta consol'a doğru götürür, kilit düğmesine basarak kapıların kilidini açar.*")
							triggerEvent('sendAme', source, "sağ elini, orta consol'a doğru götürür, kilit düğmesine basarak kapıların kilidini açar.*")
						else
							setVehicleLocked(vehicle, true)
							--exports.global:sendLocalMeAction(source, "Sağ elini, orta consol'a doğru götürür, kilit düğmesine basarak kapıları kitler.*")
							triggerEvent('sendAme', source, "sağ elini, orta consol'a doğru götürür, kilit düğmesine basarak kapıları kitler.*")
						end
					else
...
function checkLock(thePlayer, seat, jacked)
	...
	if (locked) and not (jacked) then
		cancelEvent()
		outputChatBox("Kapılar Kitlidir.*", thePlayer)
	end
end
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		if owner < 0 and faction == -1 then
			outputChatBox("(( Bu " .. carName .. " bir sivil aracıdır. ))", thePlayer, 255, 195, 14)
		elseif (faction==-1) and (owner>0) then
			...
			if ownerName then
				...
				outputChatBox("(( Bu " .. marka .. " marka aracın sahibi " .. ownerName .. ". ))", thePlayer, 255, 195, 14)
				--outputChatBox("(( Ödenmesi gereken "..exports.global:formatMoney(vergi).."TL vergi bulunmaktadır. ))", thePlayer, 255, 195, 14)
				if (getElementData(source, "Impounded") > 0) then
					local output = getRealTime().yearday-getElementData(source, "Impounded")
					outputChatBox("(( Bu " .. marka .. " haciz yiyerek çekildi: " .. output .. (output == 1 and " Day." or " Days.") .. " ))", thePlayer, 255, 195, 14)
				end
			end
		end
	end
...
--[[function antiVergi(thePlayer, seat, jacked)
	...
	if vjob == 0 then
		if (getElementData(source, "faizkilidi")) then
			...
			outputChatBox("(( Bu "..marka.." marka aracın vergi borcu ödenmediği için çekilmiştir. ))", thePlayer, 255, 194, 14)
					...
					if theVehicle then
						...
						if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") or exports.global:isPlayerSuperAdmin(thePlayer) or (exports.donators:hasPlayerPerk(thePlayer, 16) and exports.global:isPlayerFullAdmin(thePlayer)) then
							...
							if oynamaSaati < 10 then
								outputChatBox("Bu işlem için sunucuda 10 saat geçirmiş olmanız gerekmekte.", thePlayer, 255, 0, 0)
								return
							end
									...
									if exports.global:canPlayerBuyVehicle(targetPlayer) then
										--if exports.global:isPlayerFullAdmin(thePlayer) --[[or exports['carshop-system']:isForSale(theVehicle)]] then
											...
											if query then
												...
												outputChatBox("#FF0000Araç Markası : " .. getVehicleName(theVehicle) .. "  #FFFFFFYeni Sahibi :  " .. targetPlayerName .. ".", thePlayer, 0, 0, 0, true)
												outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " #FFFFFFFadlı şahıs " .. getVehicleName(theVehicle) .. " #FFFFFFarabanın tüm hakları ile üstünüze geçirdi.", targetPlayer, 255, 0, 0, true)
												outputChatBox("#FFFFFFLütfen aranızıa /park yazın ve gitmemesini sağlayın." .. getVehicleName(theVehicle) .. ".", targetPlayer, 0, 0, 0, true)
...
--[[function aracsat(thePlayer, commandName, targetPlayerName, aracFiyati)
	...
	if isPedInVehicle(thePlayer) then
		if not targetPlayerName or not aracFiyati then
			outputChatBox("SÖZDİZİMİ: /aracsat [Oyuncu ID] [Ucret]", thePlayer, 255, 194, 14)
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if oynamasaati < 10 or oynamasaatiKarsi < 10 then
					outputChatBox("Bu komutu kullanabilmek için en az 10 oynama saatinizin olması gerekiyor.", thePlayer, 255, 0, 0)
					return
				end
				local px, py, pz = getElementPosition(thePlayer)
				local tx, ty, tz = getElementPosition(targetPlayer)
					...
					if theVehicle then
						...
						if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") or exports.integration:isPlayerTrialAdmin(thePlayer) then
							if getElementData(targetPlayer, "dbid") ~= getElementData(theVehicle, "owner") then
								if exports.global:hasSpaceForItem(targetPlayer, 3, vehicleID) then
									if exports.global:canPlayerBuyVehicle(targetPlayer) then
										...
										if not (exports.global:hasMoney(targetPlayer, tonumber(aracFiyati))) then
											outputChatBox("[!] #f0f0f0" .. targetPlayerName .. " isimli kişinin yeterli parası yoktur.", thePlayer, 255, 0, 0, true)
											return false
										end
										...
										outputChatBox("[!] #f0f0f0" .. (getPlayerName(thePlayer):gsub("_", " ")) .. " isimli kişi size #" .. tostring(vehicleID) .. " ID'li '" .. getVehicleName(theVehicle) .. "' aracı " .. exports.global:formatMoney(aracFiyati) .. "₺'ye satmak istiyor. Satın almak isterseniz /satisonayla iptal etmek isterseniz ise /satisiptal yazınız. ", targetPlayer, 0, 0, 255, true)
									else
										outputChatBox("[!] #f0f0f0" .. targetPlayerName .. " isimli kişinin araç alacak yeri yoktur.", thePlayer, 255, 0, 0, true)
								...
								outputChatBox("[!] #f0f0f0Satmak istediğini kişi zaten aracın sahibi.", thePlayer, 255, 0, 0, true)
							end
						else
...
function satisOnayla(thePlayer, cmd)
	if getElementData(thePlayer, "aracSatis") then
		if cmd == "satisiptal" then
			...
			outputChatBox("[!] #FFFFFF'" .. getPlayerName(thePlayer) .. "' isimli oyuncu, satışı onaylamadı.", satanPlayer, 255, 255, 0, true)
		...
		if query then
			...
			outputChatBox("[!] #FFFFFF'" .. getVehicleName(theVehicle) .. "' aracınızı başarıyla " .. exports.global:formatMoney(satisFiyat) .. "₺'ye " .. getPlayerName(thePlayer) .. " isimli oyuncuya sattınız.", satanPlayer, 0, 255, 0, true)
			outputChatBox("[!]#FFFFFF"..(getPlayerName(thePlayer):gsub("_", " ")) .. " isimli oyuncudan " .. exports.global:formatMoney(satisFiyat) .. "₺'ye bir '" .. getVehicleName(theVehicle) .. "' satın aldınız.", thePlayer, 0, 255, 0, true)
			outputChatBox("[!] #FFFFFF'" .. getVehicleName(theVehicle) .. "' aracınızı /park etmeyi unutmayınız.", thePlayer, 255, 255, 0, true)
...
function lockUnlockOutside(vehicle)
	if (not source or exports.global:isPlayerAdmin(source)) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		...
		if (isVehicleLocked(vehicle)) then
			setVehicleLocked(vehicle, false)
			--exports.global:sendLocalMeAction(source, "Kumandaya basarak, aracın kilidini açar.* ((" .. getVehicleName(vehicle) .. "))")
			triggerEvent('sendAme', source, "kumandaya basarak, aracın kilidini açar.* ((" .. getVehicleName(vehicle) .. "))")
			...
			setVehicleLocked(vehicle, true)
			--exports.global:sendLocalMeAction(source, "Kumandaya basarak, aracın kilidini kilitler.*  ((" .. getVehicleName(vehicle) .. "))")
			triggerEvent('sendAme', source, "kumandaya basarak, aracın kilidini kilitler.*  ((" .. getVehicleName(vehicle) .. "))")
		...
		elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and getElementData( thePlayer, "faction" ) ~= 1  and not exports.global:isPlayerAdmin(thePlayer) then
			...
			if (owner==playerid and TowingReturn == 0) or (exports.global:hasItem(thePlayer, 3, dbid)) or (TowingReturn == 2) or (exports.global:isPlayerAdmin(thePlayer) and exports.logs:logMessage("[AVEHPOS] " .. getPlayerName( thePlayer ) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) then
				if (dbid<0) then
					...
					outputChatBox("Aracınız buraya, ./parklanmıştır.*", thePlayer)
	...
	elseif isElementWithinColShape( source, PershingSquareCol ) and getElementData( source, "faction" ) ~= 1  and not exports.global:isPlayerAdmin(source) then
		...
		if (owner==playerid and TowingReturn == 0) or (exports.global:hasItem(source, 3, dbid)) or (TowingReturn == 2) or (exports.global:isPlayerAdmin(source) and exports.logs:logMessage("[AVEHPOS] " .. getPlayerName( source ) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) then
			if (dbid<0) then
				...
				exports['anticheat-system']:changeProtectedElementDataEx(veh, "interior", interior)
				exports['anticheat-system']:changeProtectedElementDataEx(veh, "dimension", dimension)
				outputChatBox("Aracınız buraya, spawnlanmıştır.*", source)
				exports.logs:dbLog(thePlayer, 4, {  veh }, "PARK")
				for key, value in ipairs(destroyTimers) do
					if (tonumber(destroyTimers[key][2]) == dbid) then
						local timer = destroyTimers[key][1]
