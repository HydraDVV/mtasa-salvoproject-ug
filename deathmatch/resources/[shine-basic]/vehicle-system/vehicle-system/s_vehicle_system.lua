...
addCommandHandler("makecivcar", createCivilianPermVehicle, false, false)
...
function loadOneVehicle(id, hasCoroutine)
	...
	if row then
		...
		if veh then
			...
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "rent", tonumber(row.rent))
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "rentcost", tonumber(row.rentcost))
		...
		if (seat == 0) then
			...
			if not (enginelessVehicle[model]) then
				...
				if engine == 0 then
					...
					if (vehKey) or (owner < 0) and (faction == -1) or (playerFaction == faction) and (faction ~= -1) or (getElementData(source, "adminduty") or 0) then
					...
					setVehicleEngineState(veh, false)
					exports['anticheat-system']:changeProtectedElementDataEx(veh, "engine", 0, false)
					exports['anticheat-system']:changeProtectedElementDataEx(veh, "vehicle:radio", 0, true)
				end
			end
		end
	end
	...
	if isVehicleLocked(source) then
		...
		elseif (faction==-1) and (owner>0) then
			...
			if getElementData(source,"rent") == 0 then
				...
				outputChatBox("(( This " .. carName .. " is Rented by " .. ownerName .. ". ))", thePlayer, 255, 195, 14)
			end
...
function setEngineStatusOnEnter(thePlayer, seat)
	if (seat == 0) or (seat == 1) or (seat == 2) or (seat == 3) then
		local model = getElementModel(source)
		...
		if not (enginelessVehicle[model]) then
			if (engine == 0) then
				toggleControl(thePlayer, 'brake_reverse', false)
				setVehicleEngineState(source, false)
				mysql:query_free("UPDATE vehicles SET engine = '0' WHERE id = " .. dbid .. "")
				exports['anticheat-system']:changeProtectedElementDataEx(source, "engine", 0, true)
			elseif (engine == 1) then
				toggleControl(thePlayer, 'brake_reverse', true)
				setVehicleEngineState(source, true)
				mysql:query_free("UPDATE vehicles SET engine = '1' WHERE id = " .. dbid .. "")
				exports['anticheat-system']:changeProtectedElementDataEx(source, "engine", 1, true)
			end
		else
			toggleControl(thePlayer, 'brake_reverse', true)
...
function bugfix()
    if ( getElementType(source) == "player" ) then -- Make sure the element is a player
		...
		if vehicle then
			setTimer(setVehicleEngineState, 300, 1, vehicle, false)
			exports.global:sendMessageToAdmins("[AdmWarn] Player '" .. getPlayerName(source):gsub("_"," ") .. "' attempted to drop clothes and start a vehicle.")
		end
    end
end
addEventHandler("onElementModelChange", root, bugfix)
...
--[[			exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "realinvehicle", 0, false)
	...
	end	local Impounded = getElementData(source,"Impounded")
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 20 then
					...
					if theVehicle then
						...
						if getElementData(theVehicle,"rent")== 1 then
							outputChatBox("You can't sell a rented vehicle.",thePlayer,255,2,2)
							return
						end
